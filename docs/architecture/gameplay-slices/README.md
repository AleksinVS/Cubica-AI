...
- `GSR-050` - Estate Race: закрытое приглашение одноразово занимает сетевое место, durable credential передаётся через `HttpOnly` cookie, а authenticated SSE и полный HTTP GET восстанавливают персональную проекцию; принято для закрытой альфы, но не для каталога/production.
