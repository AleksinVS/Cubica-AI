...
- `GSR-037` - Estate Race: фундамент S1 с полным оригинальным полем из 40 клеток и конфигурацией 2–6 участников; порядок, дубли и активированные landing-сценарии остаются явно незакрытыми до отдельного типизированного proof.
- `GSR-038` - Estate Race: серверная цепочка дублей, дополнительный бросок после разрешения клетки и третий дубль с заключением; случайный начальный порядок остаётся заблокирован тем, что `core.entities.order` не принимает фактическую record-map коллекцию участников.
