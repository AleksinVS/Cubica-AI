...
Проекту нужна единая политика тестирования, которая сокращает повторную работу без ослабления контрактных, архитектурных и пользовательских гарантий. Она действует поверх существующих запускателей тестов и не меняет JSON Schema как единственный авторитетный источник правил структуры данных для манифестов.
...
- **Полный запуск после каждого изменения или отдельный nightly-контур** — дают широкое покрытие, но повторяют работу и не нужны для принятого объёма; отдельный ночной прогон не создаётся.
...
- Проверки запускаются ступенчато: самый узкий прямой тест в edit loop, затем проверка границы задачи, обязательные CI-статусы и финальная/release-приёмка. E2E выполняется только для пользовательской границы и после успешных дешёвых шлюзов.
- Целевой CI исключает повторы одного и того же покрытия, отменяет устаревшие прогоны одного pull request и измеряет длительность стадий. Тяжёлые диагностические артефакты и расширенные трассировки создаются только при ошибке.
- Целевые локальные точки запуска включают affected, failed (last-failed), profile и E2E smoke. Селектор affected является fail-closed: неизвестное, общее или критичное изменение расширяет набор до полного резерва; в CI он первоначально наблюдательный и не сокращает обязательные статусы.
- Параллелизм разрешён только для проверок с доказанной изоляцией портов, build output, временных каталогов, фикстур, переменных окружения и общего изменяемого состояния. Полные наборы, сборки и E2E в одном workspace выполняются последовательно; метка `serial` сама по себе не является межпроцессной блокировкой.
- AJV/JSON Schema, contract, security и authorization проверки остаются обязательными; оптимизация не может понизить их статус или заменить E2E более дешёвой проверкой.
...
- Защита ветки `main` не включается этим решением, а отдельный nightly-прогон не создаётся.
