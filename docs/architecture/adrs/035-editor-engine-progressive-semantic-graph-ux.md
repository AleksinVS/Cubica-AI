...
Уточнение MVP от 2026-09-19 (ADR-057): видимая диаграмма исходников не входит
в MVP. Настоящее решение сохраняет модель внутренней проекции и поведения
прежнего расширенного представления; оно не требует загружать или показывать
React Flow в новой оболочке. См. [MVP UX](../editor-mvp-ux.md).
...
Это решение определяет поведение диаграммы, а не основную поверхность редактора.
ADR-036 и ADR-057 устанавливают предпросмотр игры как центр рабочего пространства;
диаграмма остаётся дополнительным представлением authoring JSON. При расхождении
правил компоновки и открытия панелей применяются последующие решения.
