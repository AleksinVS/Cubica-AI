...
- [11. Управление кандидатами](#11-управление-кандидатами)
...
7. Поиск кандидатов является отдельной не изменяющей данные процедурой:
   детерминированная проверка образует базовый контур, а LLM-анализ может
   только предложить дополнительные кандидаты. Периодичность запуска и очередь
   рассмотрения принадлежат процессу исполнения, а не ADR.
...
- типовые кнопки и navigation controls;
...
```json
{
  "_definitions": {
    "ui.GameLocalScreen": {
      "_semantics": "Локальная оболочка экрана конкретной игры.",
      "_promptTemplate": {
        "raw": "Опишите назначение topbar-экрана, главный контент, действия и метрики.",
        "language": "ru",
        "appliesTo": "ui.GameLocalScreen"
      }
    }
  },
  "root": {
    "screens": [
      {
        "_type": "ui.GameLocalScreen",
        "id": "example-screen",
        "_label": "Example screen",
        "title": "..."
      }
    ]
  }
...
Platform-level catalog является отдельным authoring/tooling слоем. Он не живёт
в `runtime-api`, `player-web` или runtime contracts как исполнимая логика;
platform-level prototypes являются authoring-only input для компилятора и
редактора.
...
```json
{
  ...
  "_definitions": {
    "ui.GameLocalMetricBar": {
      "_extends": "ui.MetricBar",
      "_semantics": "Локальная панель метрик с предметными названиями игры."
    }
  }
}
```
...
## 11. Управление кандидатами

Аудит кандидатов не применяет изменения, не создаёт `_definitions` и не повышает
локальные прототипы автоматически. Его результат — versioned candidate records,
отчёты, явные suppression records и запросы на ручное повышение. Быстрые проверки
должны быть детерминированными; LLM-анализ может только предложить кандидата и
никогда не является блокирующим источником изменения.
...
Отсутствие периодического аудита не блокирует редактирование, preview или Save;
оно может давать неблокирующую диагностику. Расписание и представление этой
диагностики не являются частью ADR.
...
- нужен отдельный контур отчётов, чтобы поиск кандидатов не смешивался с
  применением изменений в редакторе;
- автору нужна видимая диагностика состояния кандидата, не создающая второй
  источник истины;
