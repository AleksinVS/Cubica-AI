...
> Уточнение ADR-048: `generation.prompt` в этом ADR относится только к визуальному design artifact или визуальному региону. Он не является элементным authoring-промтом `_prompt`, который описывает содержимое, поведение и методический смысл конкретного game/UI экземпляра.
