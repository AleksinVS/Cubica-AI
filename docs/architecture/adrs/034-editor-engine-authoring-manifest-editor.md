...
- **Дата**: 2026-05-22
**Status:** Accepted
- **Принят**: 2026-09-19 — PM подтвердил описанную модель поручением исправить документацию после её обсуждения.
- **Авторы**: Codex
...
Уточнение MVP от 2026-09-19 (ADR-057): пользовательские дерево манифеста,
диаграмма исходников и JSON-редактор исключены из MVP. Приведённые ниже
технические представления описывают внутреннюю модель и прежний расширенный
интерфейс, а не обязательные экраны MVP. Пользователь работает через предпросмотр,
чаты и YAML-проекцию элемента; см. [MVP UX](../editor-mvp-ux.md).

Решение понимается так: Cubica использует `editor-engine` как универсальный движок редактирования authoring-манифестов. Authoring-манифест - исходный JSON-документ для разработки игры или UI, который компилируется в runtime-манифест по ADR-030.
...
ADR-036 и ADR-057 уточняют рабочее пространство: основной поверхностью является
предпросмотр игры, а дерево сущностей, диаграмма, JSON и панель свойств —
согласованными представлениями исходников. Настоящее решение фиксирует модель
редактирования; актуальная компоновка и поведение выбора определяются этими
