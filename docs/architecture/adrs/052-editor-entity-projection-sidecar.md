...
Уточнение PM от 2026-09-21 принято в [ADR-108](108-prototype-defined-semantic-yaml-editing.md):
прототип выбирает значимые свойства проекции; проверенный контекст предпросмотра
связывает отображение с конкретным содержимым. Скрытая карта сохраняет владельцев
полей, а обычная запись в экземпляр не перенаправляется в источник наследования.
Новый сохраняемый editor-manifest не вводится.
