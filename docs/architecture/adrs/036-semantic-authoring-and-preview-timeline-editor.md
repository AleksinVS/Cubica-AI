...
Уточнение MVP от 2026-09-19 (ADR-057): основная поверхность дополнена отдельными
чатами, меню сценария заменяет постоянную навигационную панель, дерево/JSON
не показываются. Приняты три блока панели элемента и рисование; полная пауза
и точный снимок прохождения не должны подменяться player-projected fixtures.
Подробные продуктовые требования — [MVP UX](../editor-mvp-ux.md).
