...
Уточнение от 2026-09-21 (MVP ADR-057/108): панель элемента показывает три смысловых
части одного текстового поля — разовый промт, сохраняемое авторское описание и актуальную
YAML-проекцию. [ADR-108](108-prototype-defined-semantic-yaml-editing.md) определяет
состав проекции прототипом, естественно-языковые описания правил и скрытые
технические привязки. YAML сохраняется как свободный структурированный промт.
Разовый промт не становится частью `_prompt`. Авторское описание
сохраняет роль невосстановимого намерения, YAML пересобирается из JSON. Указание
ниже на единый compiled prompt описывает сборку контекста, а не обязательное
единое поле ввода. ADR-057 допускает детерминированный быстрый путь для
однозначной правки; обязательный вызов ИИ на каждую текстовую правку не нужен.
См. [MVP UX](../editor-mvp-ux.md).
