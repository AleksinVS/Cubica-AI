...
2. Динамическая часть является YAML-проекцией выбранного JSON-узла. Проекция - это человеко-читаемое представление данных для редактора, а не новый формат хранения.
...
3. Динамическая часть показывается пользователю в YAML-форме с русскими названиями полей.
...
Dynamic YAML projection (динамическая YAML-проекция) - это временное YAML-представление authoring JSON-узла с локализованными названиями полей.

Пример:

```yaml
Карточка выбора:
  Идентификатор: card-17
  Текст на лицевой стороне: "Поговорить с Аленой"
  Действие при выборе: choice.accept-alena
...
Projection builder должен строить sidecar source map, который не обязательно виден пользователю:
...
- `_label`, `_semantics`, `title`, `name`, `description`;
