...
> **Статус (решение PM 2026-07-17): отложенный design reference, не действующая
> архитектура.** Redis — резерв только для кэша чтения (write-through;
> PostgreSQL — единственный источник истины; write-behind отклонён).
> Блокировки выполняет только PostgreSQL (`SELECT FOR UPDATE NOWAIT`,
> ADR-005/059) — разделы этого документа о блокировках отменены. Внедрение
> кэша требует именованного триггера и актуализации документа.
