...
- Фактически реализованный legacy-путь `manifest-data` пока использует guard и schema-defined `deterministic.effects[]`; он заморожен для новых механик и подлежит полной миграции.
- Целевой путь — actor-scoped Game Intent → Cubica Mechanics IR: клиент передаёт только `actionId + params + commandId`, а runtime выполняет раздельные `availability`/`authorization` и типизированную атомарную транзакцию через явно закреплённые алгоритмические модули.
...
- Agent output may include narration, diagnostics, a `CubicaSurface` and selection of an actor-scoped Game Intent. The currently implemented direct state-effects field is legacy and must migrate; the agent does not submit arbitrary Mechanics IR or patches.
...
- Любая доступная запросам коллекция имеет тип, видимость, стабильный ключ и максимальную ёмкость. Профиль бюджета ограничивает статическую стоимость, runtime-работу, память и выход; манифест не может повышать платформенный предел.
...
- Все текущие пути изменения сессии используют одну транзакцию на одном соединении и пессимистичную блокировку `SELECT FOR UPDATE NOWAIT`; условная запись по ожидаемой версии дополнительно защищает от потери обновления. `state_version` всегда увеличивается ровно на единицу, включая restore предпросмотра; при restore назад перемещается только `last_event_sequence`, а старое состояние становится новым устойчивым снимком. Хранилище в памяти разрешено только как явно выбранный dev/test-адаптер.
...
- **ADR-046 (AI-Driven Game Runtime Mode, amended by ADR-084):** фиксирует `ai-driven` и `hybrid` execution modes, где Agent Runtime может быть обязательной частью игрового исполнения. Narration, diagnostics и Cubica Surface остаются валидируемым агентным результатом; целевая долговременная мутация больше не принимает произвольные agent-produced state effects и сходится в actor-scoped Game Intent → Mechanics IR. Фактически реализованный старый field остаётся миграционным долгом до переключения.
- **ADR-047 (AI Agent Safety Remediation Gates, amended by ADR-084):** accepted safety gates перед production Agent Runtime: human approval хранится как Cubica approval envelope, rejected Agent Turn не применяет state changes, `allowedCapabilities` является исполняемым runtime allowlist, Surface actions проверяются channel policy, а non-Web projections fail closed. Allowlist изменения состояния содержит Game Intents, а не низкоуровневые IR-операции.
...
- **ADR-061 (Action Parameters, extended by ADR-074):** accepted параметры действий манифеста: действие объявляет `paramsSchema` (плоский объект со скалярными свойствами, явный `additionalProperties: false`), клиент передаёт `params` в `POST /actions`, runtime валидирует их Ajv до guards, провалидированные значения доступны выражениям через ветку `params` контекста JsonLogic и попадают в состояние только явными эффектами. Параметры по умолчанию являются инертными данными и не интерпретируются как пути, ключи или выражения. Узкое исключение ADR-074 — строка с заранее объявленной `x-cubica-ref`: runtime разрешает ее только в заданной манифестом коллекции/сети и повторно проверяет ресурс; даже такая ссылка никогда не становится путем или кодом. Потребители: игры-симуляции (ADR-062), торги/аукционы настольных игр (ADR-058 §2.7), проекция `availableActions` (ADR-060) и динамические игровые объекты (ADR-074).
...
- **ADR-076 (Unified Cross-Scope Economic Transfers):** accepted единый окончательный контракт `metric.transfer`: стороны используют `scope: bank | player | state`, участник адресуется безопасным `playerRef` и `metricId`, общий или командный баланс — статическим JSON Pointer, политика недостатка средств называется `onInsufficient: "fail"`. Клиент не конструирует путь к балансу участника, перевод атомарен и не создаёт отрицательный баланс; прежние поля не являются частью принятого контракта.
...
- **ADR-072 (Declarative Dynamic Transport Network Board):** accepted общая модель изменяемой транспортной сети. Game-манифест владеет начальным графом, канонической геометрией, выбранной стратегией планирования, состояниями и правилами игры; runtime владеет связностью, создаваемыми узлами/связями, допустимостью операций и атомарными изменениями; UI владеет представлением и вводом намерения. Текущий отдельный эффект активации строительства является только фактическим миграционным долгом: по ADR-084 он заменяется общим запросом объектов, сравнением времени и атомарной командой, transport authoring-макрос компилируется в IR, а старый эффект и адаптер удаляются при общем переключении. Направленность, конкретная задержка, стоимость, финансирование и границы фаз остаются декларациями игры.
...
- **ADR-084 (Game Intents, Capability Catalog And Typed Mechanics IR, Accepted):** внешний изменяющий контракт — Game Intent в терминах игры, но его каноническим полем остаётся существующий `actionId`; отдельный `intentId` не вводится. Клиент передаёт `actionId + params + cli_ commandId + expectedStateVersion`, principal приходит только из аутентификации, actor разрешается сервером, а доверенный `playerId` в теле отсутствует. Command ledger индексируется стабильным principal, поэтому retry безопасен и в хотсите после смены активного игрока. Presenter использует опубликованные UI-bindings и не выбирает действие из предметного состояния. Неизменяемые action definitions и actor/state-scoped availability являются разными проекциями. ИИ-разработчик и editor используют широкий operation catalog через build-time mechanics packs; UI packs независимы, runtime modules могут быть общими платформенными либо явно разрешёнными изолированными extensions игры. Cubica Mechanics IR `cubica.dev/mechanics/v1alpha1` разделяет чистые query/assert/algorithm, изменяющие command и структурные control nodes; состояние, случайность и события меняют только команды. `moduleLock` отдельно закрепляет `moduleId`, точную `moduleVersion`, `artifactHash` и версии алгоритмов. Session pins immutable `bundleHash`; state/random/events/audit/internal receipt фиксируются одним commit. Внутренний планировщик выводит `sys_ commandId` из schedule/occurrence, не использует неизвестный заранее `expectedStateVersion` и повторно проверяет trigger под session lock; префикс не даёт полномочий. Раскрытие `planHash` для учебных данных не считается критичным и не блокирует выпуск, но публичная receipt остаётся отдельной проекцией. После lockstep migration удаляются `deterministic.effects[]`, 29 вариантов, old executor/templates, доверенный клиентский `playerId`, `params/payload` duality, client command maps, direct AI effects и постоянные adapters.
  ...
  effects ADR-046/047 являются legacy, а не целевым Agent Turn mutation
  contract.
