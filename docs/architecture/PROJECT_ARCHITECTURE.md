...
- Draft ADR-088 предлагает отделить продуктовые знания пользовательских помощников от принятой инженерной вики. Полные диалоги и служебный процесс каноничны в PostgreSQL, а подтверждённый синтез — в пакетах Markdown по строгому профилю Google Open Knowledge Format 0.1 Draft. Пользователь видит одну динамически собранную поверхность, но личный профиль, командный проект, опубликованная версия и проведение остаются пакетами с разными владельцами, правами и сроками. Проектные пакеты могут жить в Game Repository/Git, а личным и клиентским пакетам нужно управляемое Markdown-хранилище с физическим удалением. Первая версия использует иерархические `index.md`, метаданные, прямые ссылки и точное чтение; FTS добавляется только по измеримому сигналу, а векторы — после доказанного недостатка FTS. Актуальные Game Repository, Portal и Runtime данные сильнее wiki; вывод модели только предлагает правку, найденный текст не меняет права, а удаление источника распространяется на зависимые знания. Решение остаётся черновиком до согласования категорий и сроков данных, физического хранилища личных пакетов, переноса между ролями, внешней обработки и единой платформенной личности.
...
- **ADR-088 (Product Knowledge System Boundaries, Draft):** предлагаются две системы верхнего уровня: принятая инженерная вики и отдельная продуктовая способность. В продуктовой системе точный текст диалогов и процесс синтеза хранятся в PostgreSQL, а подтверждённые знания — в пакетах Markdown по строгому профилю OKF 0.1 Draft. Единый пользовательский экран динамически объединяет только разрешённые пакеты: личный, проектный, опубликованной версии и конкретного проведения. Пакеты имеют разных владельцев и сроки; личные и клиентские данные не помещаются в бессрочную Git-историю. Права вычисляются до перечисления пакетов, а frontmatter и пути не считаются доступом. Модель предлагает правку только из реплик пользователя или авторитетных событий; подтверждение, происхождение и ожидаемая ревизия обязательны. В первой версии `index.md` и точное чтение заменяют контентный индекс; FTS и векторы вводятся последовательно только после измеримого недостатка. Предметные источники Cubica сильнее wiki, найденный текст не меняет инструменты, а удаление источника очищает зависимый синтез и производные. Отдельные графовая/векторная базы и микросервис отклоняются. Решение не принято: открыты категории и сроки данных, управляемое Markdown-хранилище, перенос личных настроек между ролями, внешний поставщик и единая модель личности.
