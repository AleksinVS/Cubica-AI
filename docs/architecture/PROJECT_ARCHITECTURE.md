...
- **ADR-052 (Editor Entity Projection Sidecar, Draft):** редактор получает project-level projection sidecar для единой editor entity, которая связывает logic, content, state, view, design and plugin source pointers across multiple authoring files. Sidecar является tooling-only индексом, может храниться как `games/<gameId>/authoring/editor.entities.json`, не копирует игровые данные, не требуется compiler/runtime/player слоям, пересобирается из authoring sources и все player-visible изменения записывает обратно через `EditorChangeSet` в исходные authoring manifests.
