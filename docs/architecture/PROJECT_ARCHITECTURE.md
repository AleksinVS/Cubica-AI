...
- `games/antarctica/game.manifest.json` уже покрывает bounded gameplay slice records до boundary `stepIndex = 30`; архитектурное правило для этих механик закреплено в ADR-024, а step-specific delivery details вынесены в `docs/architecture/gameplay-slices/`.
...
- В этом canonical slice bounded gameplay records `GSR-020`..`GSR-026` уже реализованы и доводят mainline до boundary `stepIndex = 30`; архитектурные ограничения для такого моделирования зафиксированы в ADR-024.
