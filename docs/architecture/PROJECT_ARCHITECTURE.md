...
- **ADR-069 (Managed External Skill Adaptation):** accepted внешние навыки подключаются только через управляемую проектную адаптацию. Компактный декларативный профиль совместимости с устойчивыми rule IDs, тегами и отпечатками источников дополняется пополняемой памятью ошибок и практик; скрипты лишь собирают механические сигналы и минимальный пакет, а семантическое решение принимает агент с правом точечно читать проект. Результат кэшируется по хэшу навыка и версиям политики, памяти и адаптера. Upstream проверяется пакетно не чаще раза в 30 дней, кроме ручного запроса или события безопасности, и никогда не заменяет активную версию автоматически. Активной становится материализованная версия без конкурирующих инструкций; происхождение, лицензия, способность и статус фиксируются в реестре, одна процессная способность по умолчанию имеет одного владельца. `$cubica` остается явно включаемым, остальные допущенные навыки могут включаться автоматически.
- **ADR-070 (Shared Project Skill Catalog):** accepted проектные навыки имеют единое каноническое расположение `skills/`, одинаковое для всех агентов. Внутренние навыки Cubica именуются `C_*`; адаптированные внешние навыки сохраняют предметные имена и происхождение в реестре. Среды агентов используют мосты обнаружения к этому каталогу, а не собственные копии: для Codex это символические ссылки из `.codex/skills/`. `$cubica` по-прежнему требует явного включения, остальные допущенные специализированные навыки могут включаться автоматически.
...
- **ADR-073 (Phaser Interactive Board Surface, Accepted):** общий лениво загружаемый Phaser-host расширяется отдельной поверхностью `interactiveBoardSurface` для пошаговых 2D-полей. Phaser-плагин игры владеет картой, камерой, drag/drop и анимацией подтвержденного состояния; runtime остается единственным владельцем правил, денег, сети и случайности; этапы хода, карточки, расчеты, журнал и методика остаются доступным DOM/React-интерфейсом. Пользовательский ввод сцены является только намерением и применяется после серверной проверки. Гибридная граница подтверждена PM 2026-07-11.
