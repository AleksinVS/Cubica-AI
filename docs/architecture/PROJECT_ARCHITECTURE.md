...
- **ADR-071 (Facilitated Training Sessions, Draft):** предложена общая модель учебной сессии, в которой игровые места отделены от персонала с ролями `facilitator`, `assistant` и `observer`. Переходы фаз, методические паузы и интервенции выполняются только объявленными действиями манифеста, проверяются по роли и журналируются; ведущий не получает произвольный API изменения состояния, а игровые события не превращаются автоматически в психологическую оценку участника. Для первой версии «Карт, денег, поездов» выбран один локальный общий экран и отдельная сетевая идентичность персонала не нужна; решение ожидает согласования минимального состава общих ролей и политики хранения методических заметок.
- **ADR-072 (Declarative Transport Network Board, Draft):** предложена общая декларативная модель дискретной транспортной сети. Game-манифест владеет узлами, связями, доступностью, вместимостью и правилами пути; UI-манифест владеет фоновым ассетом и координатами; runtime проверяет перемещение и детерминированно рассчитывает кратчайшее расстояние. Первый срез использует авторский каталог кандидатных дорог и мест полустанков, а не выводит топологию из пикселей и не доверяет клиентскому расчету. Решение ожидает согласования PM по границе свободного строительства и расчету стоимости.
