...
> - **Система продуктового контекста** — предлагаемый многопользовательский контур, который собирает для агента разрешённые актуальные данные и долговременные сведения о пользователе, игровом проекте или мероприятии; это не инженерная проектная вики и не источник игрового состояния.
...
- Draft ADR-088 предлагает отделить пользовательскую долговременную память от принятой инженерной вики. Общее ядро контрактов, происхождения, FTS/векторного объединения и оценки допускается переиспользовать, но инженерный, авторский и публичный домены не делят базы, индексы, кэши, роли или пространства идентификаторов. Авторский контекст связывает арендатора, пользователя, проект и версию; контекст проведения — ведущего, покупку, заказчика, мероприятие и runtime-сессию. PostgreSQL, RLS, полнотекстовый поиск и `pgvector` достаточны для первой формы; отдельные графовая/векторная базы и микросервис не вводятся без измеримого основания. Актуальные Game Repository, Portal и Runtime данные сильнее памяти; вывод модели остаётся кандидатом до подтверждения, найденный текст не меняет инструменты и права, а удаление источника каскадно очищает производные. Решение остаётся черновиком до согласования категорий персональных данных, сроков, переноса между ролями, внешних поставщиков и единой платформенной личности.
...
- **ADR-088 (Product Context System Boundaries, Draft):** предлагается общее технологическое ядро контекста с раздельными инженерным, авторским и публичным доменами данных. Инженерная Git/Markdown-вики ADR-082 не хранит пользователей; продуктовый контекст использует PostgreSQL и разделяется на авторскую область проекта/версии и область проведения для покупки/заказчика/мероприятия/сессии с разными ролями БД, индексами и кэшами. Предметные источники истины — контур личности, Game Repository, Portal и Runtime API — сильнее памяти. Диалоги отделены от подтверждённых утверждений и производных сводок/эмбеддингов; кандидаты модели не становятся фактами без основания, исправление использует `supersedes`, удаление каскадно очищает производные. Проверенная сервером область входит в поисковый запрос, а RLS независимо ограничивает видимые строки; найденный текст остаётся недоверенными данными и не меняет права или инструменты. Отдельный граф, векторная БД и микросервис отклоняются до измеримого ограничения. Решение не принято: открыты политика персональных данных и сроков, перенос личных настроек между ролями, внешний поставщик, служебный доступ и единая модель личности.
