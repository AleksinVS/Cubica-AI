...
- **ADR-076 (Unified Cross-Scope Economic Transfers):** accepted единый окончательный контракт `metric.transfer`: стороны используют `scope: bank | player | state`, участник адресуется безопасным `playerRef` и `metricId`, общий или командный баланс — статическим JSON Pointer, политика недостатка средств называется `onInsufficient: "fail"`. Клиент не конструирует путь к балансу участника, перевод атомарен и не создаёт отрицательный баланс. Тестовый пакет `cards-money-trains-mock` мигрирован 2026-07-12; старая форма удалена из общей схемы и Runtime, временный разрыв `LEGACY-0050` закрыт.
- **ADR-077 (Serialized Editor Session Mutations, Accepted):** любой пользовательский Save требует действующую editor-сессию, а Save, Restore, Close и разрушительный GC одной сессии выполняются под общей межпроцессной арендой. GC под арендой повторно проверяет Git status, удаление ограничено подтверждённым `.tmp/editor-worktrees` root, а успешное продвижение долговечной версии считается точкой фиксации даже при последующей ошибке вспомогательных метаданных. Контракт подтверждён PM и реализован 2026-07-12; разрыв `LEGACY-0051` закрыт после конкурентных тестов и production E2E.
...
- **ADR-071 (Facilitated Training Sessions):** accepted общая модель учебной сессии отделяет игровые субъекты от персонала; переходы фаз, методические паузы и интервенции выполняются только объявленными ролевыми действиями и журналируются. Первая версия «Карт, денег, поездов» имеет одну доверенную локальную роль `facilitator`: ведущий управляет командами на общем экране, ассистент не получает отдельной runtime-идентичности, свободные персональные заметки не сохраняются. Сетевые `assistant`/`observer` отложены до отдельного потребителя.
- **ADR-072 (Declarative Dynamic Transport Network Board):** accepted общая модель изменяемой транспортной сети. Game-манифест владеет начальным графом, каноническими координатами узлов, контурами областей и правилами; runtime владеет создаваемыми узлами/связями, движением, активной вместимостью, загрузкой груза, проверяемым групповым сцеплением/отцеплением и кратчайшим путем; UI владеет фоном, стилем и вводом намерения. Доставка атомарно начисляет объявленную выплату владельцу вагона и тариф владельцу локомотива по кратчайшему открытому пути, используя безопасные идентификаторы владельцев и только статические пути манифеста. Резервная/проданная техника не занимает терминал. Ведущий строит прямую дорогу между двумя существующими станциями/полустанками либо создает полустанок во внутренней точке дороги с атомарным разделением связи. Полная распределенная оплата списывается атомарно; несколько объектов разрешены до отдельного завершения фазы. Все связи первой версии двусторонние; клиентский подсчет не является источником истины.
- **ADR-073 (Phaser Interactive Board Surface, Accepted):** общий лениво загружаемый Phaser-host расширяется отдельной поверхностью `interactiveBoardSurface` для пошаговых 2D-полей. Phaser-плагин игры владеет картой, камерой, drag/drop и анимацией подтвержденного состояния; runtime остается единственным владельцем правил, денег, сети и случайности; этапы хода, карточки, расчеты, журнал и методика остаются доступным DOM/React-интерфейсом. Пользовательский ввод сцены является только намерением и применяется после серверной проверки. Обычные кнопки важных действий поля получают список через отдельную необязательную точку вклада `registerAccessibleBoardActionsProvider(gameId, provider)`: чистый `AccessibleBoardActionsProvider` читает только доступный игроку снимок и каноническую серверную доступность ADR-079, не импортирует Phaser и не вычисляет законность сам. Модуль декларативно объявляет эту точку вклада в `plugin.json`, а общая JSON Schema проверяет её до сборки. `player-web` вызывает поставщик независимо от загрузки движка, поэтому сбой Phaser отключает холст, но не клавиатурный путь; путь в UI-манифесте отклонён. Метод API 2.0 необязателен: новый пакет проверяет его наличие и не падает на старом хосте с API 2.0. Старый обработчик сцены не удовлетворяет инварианту аварийной доступности и сохранён только как помеченный устаревшим резерв до следующей несовместимой версии. Гибридная граница подтверждена PM 2026-07-11, независимый доступный путь — 2026-07-13.
...
- **ADR-080 (Declarative Map-First Workspace, Accepted):** UI-манифест может явно выбрать `layout_mode: "map-first"` и распределить прямые области экрана по ограниченным `workspaceSlot`. Единственный слой `board` заполняет рабочую область под интерфейсом; статус, боковые панели, нижние действия, управление камерой и модальные слои располагаются общей оболочкой без игровых CSS-эвристик, произвольных координат и `z-index`. Старые компоновки остаются совместимыми; DOM-действия поля не зависят от запуска Phaser, а геометрия правил не выводится из фонового изображения. Контракт не навязывает один формат подложки: для «Карт, денег, поездов» принят оптимизированный WebP для художественной карты и отдельные векторные данные для контуров и интерактивной геометрии; замена разрешения подложки не меняет канонические координаты.
