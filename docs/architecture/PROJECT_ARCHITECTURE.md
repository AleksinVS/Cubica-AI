...
- ADR-079 фиксирует общую серверную проекцию доступности действий: каждый публичный снимок сессии сообщает безопасный статус объявленных действий для текущего состояния и доверенной роли, используя тот же guard-вычислитель, что и dispatch. Клиенты блокируют недоступные действия и повторный ввод во время запроса, но runtime всегда повторно проверяет команду; внутренние пути и значения состояния клиенту не раскрываются. Параметрические действия остаются доступными для выбора, пока окончательная проверка ожидает их параметры.
...
- **ADR-058 (Turn-Based Board Game Platform Capabilities):** accepted пакет общих платформенных возможностей пошаговых настольных игр: детерминированная случайность с зерном (`random.roll`, PRNG-состояние в `state.secret.random`), колоды (`deck.shuffle`/`deck.draw` со скрытым порядком), состояние «на игрока» (`state.playersTemplate` → `state.players.<playerId>` по структуре ADR-011, scope `player`, подстановки `{{actor}}`/`{{activePlayer}}`), модель хода (`state.public.turn`, эффекты `turn.next`/`turn.repeat`/`turn.phase.set`, guard `turn`), экономические операции (`metric.transfer`, `metric.set` с JsonLogic), объяснимый групповой итог (`ranking.compute`, баланс плюс объявленная стоимость объектов, равенство без искусственной развязки) и ограниченное условное ветвление (`when`/одноуровневый `branch`) плюс жизненный цикл игрока и `endConditions`. Первый GSR-034 реализует RNG, участников, минимальный ход, движение и экономику; полный mock-срез «Карт, денег, поездов» дополнительно доказал колоды и рейтинг нейтральными фикстурами. Player-facing проекция уже удаляет платформенные `secret.random`/`secret.decks`; полное удаление legacy game-owned secret из HTTP-снимка ожидает миграции Antarctica (`LEGACY-0049`). Торги/аукционы остаются механиками уровня манифеста (ADR-024); доставка настольных игр идёт последовательно: хотсит → сетевой мультиплеер (реализация ADR-011) → ИИ-оппоненты (`hybrid` по ADR-046). Детальный источник — `docs/architecture/board-game-platform-design.md`.
- **ADR-059 (Multiplayer Realization In Modular Monolith):** accepted посадка принятой модели мультиплеера ADR-011 на текущий модульный монолит: PostgreSQL-хранилище сессий (ADR-005) как предусловие, модель участников (seats) с join-токенами и `kind: human|agent`, хотсит как вырожденный случай той же модели, очередь `session_events` с последовательной обработкой под advisory lock, WebSocket endpoint в `runtime-api` и единственный строитель персональной проекции (playerView) для REST/WebSocket/агентов. Каждый `POST /actions` передаёт `expectedStateVersion`; runtime сравнивает её под исключительной блокировкой до обработчика и отвечает HTTP 409 без мутации на устаревший или повторный запрос. Router/Web Gateway не выделяются до подтверждённых operational boundaries (ADR-017).
...
- **ADR-079 (Server-Projected Action Availability, Accepted):** runtime добавляет в каждый публичный снимок сессии безопасную проекцию `available` / `unavailable` / `parameter-dependent`, вычисленную тем же guard-механизмом, который защищает dispatch. Web/Phaser блокируют недоступные и повторные команды, но не становятся источником правил; технические guard-диагностики и секретное состояние не раскрываются.
...
- **ADR-081 (Server-Planned Region-Minimal Roads, Accepted):** для явно включённой транспортной модели runtime получает только два конца и параметры объявленного действия, сам использует версионированный навигационный граф опубликованных областей и минимизирует число непрерывных региональных сегментов. Политика разрешения равных маршрутов является частью версии планировщика; первая версия использует воспроизводимый `session-random`. Дорога сохраняет каноническую ломаную, упорядоченные сегменты, стоимость и версии геометрии/алгоритма; прямые старые связи читаются как ломаная из двух точек. `region-segment-minimum-v1` поддерживает простые неперекрывающиеся полигоны без отверстий, точные общие границы и необязательное исключение областей через объявленный путь состояния; непроверенная или неподдерживаемая геометрия отклоняется без автоматической починки.
