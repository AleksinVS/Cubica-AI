...
|------|---------|-----------|
| `plugins/arctic/contracts.ts` | Define `ArcticGameState` and content interfaces | Medium — depends on game complexity |
| `plugins/arctic/state-resolvers.ts` | Implement all resolver functions | Medium — mostly follows Antarctica pattern |
| `plugins/arctic/register.ts` | Implement `ResolverFactory` + side-effect registration | Low — boilerplate |
| `presenter/arctic-config-data.ts` | Define serializable `GameConfigData` (metrics, images, keys) | Medium — requires asset paths and metric definitions |

### 5.2 Mandatory Files to Modify

| File | Change | Risk |
|------|--------|------|
| `plugins/register-games.ts` | Add import | Low |
| Server Component (page.tsx or similar) | Provide correct `GameConfigData` for new gameId | Low |
...
6. Updates `plugins/register-games.ts` with the import
...
| File | Lines | Role |
|------|-------|------|
| `plugins/antarctica/contracts.ts` | ~80 | GameState and content interfaces |
| `plugins/antarctica/state-resolvers.ts` | ~170 | State resolution functions |
| `plugins/antarctica/register.ts` | 187 | ResolverFactory + registration |
| `presenter/antarctica-config-data.ts` | ~50 | Hardcoded config data |
...
- [ ] Create `plugins/arctic/contracts.ts` with `ArcticGameState` and content interfaces
- [ ] Create `plugins/arctic/state-resolvers.ts` with resolver functions
- [ ] Create `plugins/arctic/register.ts` with `ResolverFactory` + `registerGameResolvers("arctic", ...)`
- [ ] Create `presenter/arctic-config-data.ts` with metrics, images, screen keys
- [ ] Update `plugins/register-games.ts` to import the new register module
- [ ] Update server component to provide correct `GameConfigData` for the new gameId
