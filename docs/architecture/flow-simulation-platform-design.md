...
1. **Параметры — инертные данные.** Значение параметра никогда не
   интерпретируется как JSON Pointer, идентификатор метрики/действия/объекта,
   ключ объекта или JsonLogic-выражение. Все пути (`storePath`, `patches[].path`),
