...
- применять весь результат атомарно и повторять уже зафиксированный исход из
  состояния, событий и квитанции.
...
6. Любая ошибка откатывает state, события и версию сессии; полученное для
   незафиксированной попытки случайное значение не сохраняется.
...
Закрытая `record-map` коллекция вместо `objectType/facets/attributes` объявляет
логические поля над ограниченными путями записи и использует ключ map как
устойчивый ID. Общий `core.entities.select` выбирает такие записи только по
объявленным полям из секции selector `attributes`; `objectTypes` и `facets`
для records отклоняются checker и runtime. Полученная выборка совместима с
`core.entities.order`: текущие и связанные поля читаются тем же типизированным
механизмом чтения, а результат переставляет только исходные ID и не создаёт
копий записей.

Исполнимые endpoints и collections с разной аудиторией по общему правилу не
могут перекрывать один путь хранения отношением «родитель — потомок». Одно
узкое исключение позволяет публичной закрытой `record-map` с `stableKey:
map-key` описать ровно весь корень `players`, а отдельному исполнимому
actor-endpoint — персональный лист
`players/{context: actor}/<literal+>`. Публичные пути полей не могут совпадать
с листом, быть его предками или потомками. Одинаковые или вложенные пути двух
actor-endpoints запрещены: лист должен иметь единственного владельца типового
контракта. Static/binding/повторная динамика, широкий endpoint,
`projection-only`, server и любое другое перекрытие остаются запрещены.

Runtime исключает из closed-record проверки коллекции только такой объявленный
лист и независимо проверяет его endpoint-тип у каждого материализованного
actor на входном и итоговом состоянии. Неизвестный сосед и неверный тип
по-прежнему отклоняются. Селекторы, `value.item`, порядок и подсчёт читают
только публичные логические поля, а проектор удаляет персональный лист из
публичной ветви и возвращает его лишь подтверждённому владельцу; другой actor и
анонимный наблюдатель его не получают.
...
единственной категорией, которая получает случайный результат. Результат
приходит от криптографического серверного источника без сохраняемого состояния
генератора.
Bulk command применима ко всем targets или ни к одному; partial success
запрещён.
...
командами, потому что создают изменяющий игровой результат. Generic
`algorithm.call` с произвольными аргументами запрещён.

`sequence`, ограниченные `if`/`match` и обработка bounded-набора являются
структурными узлами плана, а не новой категорией операции. Принятый
`core.entities.each` обходит только результат предшествующего
`core.entities.select` либо его проверенную производную
`core.entities.order`. Производная сохраняет исходную коллекцию и статическую
верхнюю границу; checker поэтому знает максимальное число объектов, умножает
на него стоимость тела, а runtime сохраняет установленный производной порядок;
для обычной неупорядоченной выборки он использует канонический порядок ID. Тело
содержит не более шестнадцати существующих шагов; вложенный `each`, рекурсия, `goto` и
неограниченные циклы запрещены.

Внутри тела `value.item` читает поля или устойчивый идентификатор текущего
объекта. Результаты шагов тела видны только следующим шагам той же итерации,
очищаются перед следующим объектом и не становятся внешними результатами
плана. Поэтому условно пропущенный шаг не может случайно получить результат
предыдущей итерации.
...
обрабатывается в каноническом порядке. Для `core.entities.each` вся выборка и
всё тело остаются частью той же candidate transaction: ошибка одного
вложенного шага отменяет изменения всех уже обработанных объектов. Долговечный
защищённый аудит сохраняет ограниченный итог внешнего `each` и обычные
безопасные записи выполненных шагов тела с областью итерации. Их число и
размер входят в статический и runtime-бюджеты; локальная карта результатов
итерации наружу не публикуется.

Runtime формирует новое состояние, events и audit на пробной копии. Commit
выполняется один раз после успеха всех шагов. Ошибка:

- отбрасывает candidate state;
- не оставляет случайного результата;
...
- state, events, audit и внутренняя receipt сохраняются одним commit; состояние
  генератора в сессию не записывается.
...
сохраняет state, events, audit, receipt и отметку о потреблении occurrence.
Состояние генератора в эту транзакцию и игровой снимок не входит.
...
правило сравнения. Полное равенство разрешается либо стабильным ID элемента
коллекции, либо серверной случайностью только внутри всей равной группы.
Системное время доступно только как записанное событие.

Независимые подсистемы используют именованные назначения случайности. Имена
помогают проверять смысл и модульную привязку операции, но не создают
сохраняемые потоки.
Опубликованный IR получает hash по каноническому JSON; session закрепляет один
content-addressed `bundleHash`, который включает планы, action catalog,
schemas, compiler, modules, budgets и версию поставщика случайности. Одного IR
hash недостаточно.
...
query/command modules; random roll использует общий доверенный серверный
поставщик и объявленное назначение выбора. Старый JsonLogic не исполняется
runtime как параллельный язык.
...
- сессия закрепляет неизменяемый `bundleHash`; точный повтор команды
  возвращает сохранённую квитанцию, а state, события, защищённый ограниченный
  Mechanics-аудит и квитанция фиксируются атомарно; состояние генератора не
  входит в сессию;
