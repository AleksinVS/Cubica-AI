...
Статус: Requires rebaseline (игровое поведение сохранено; исполнительная форма
механик заменена ADR-084)
Дата: 2026-07-06
...
Документ написан для исполнителя-агента (в том числе со слабой LLM). Числа,
формулы, контент, сценарий, UI и приёмочные результаты разделов 4–9 остаются
нормативными требованиями поведения.

> [!IMPORTANT]
> Исполнительная JSON-форма §3, §5, §6, §9 и §11 заменена ADR-084. Примеры с
> `handlerType`, runtime guards, `effects[]`, `when`, `metric.set` и JsonLogic
> нельзя копировать в игру или возвращать в платформу. Перед реализацией все 15
> действий должны быть перепривязаны к Game Intents и типизированному Mechanics
> IR. Если актуальный язык не выражает требуемое поведение, это фиксируется как
> потребность общего модуля либо изолированного расширения игры; старый executor
> не является допустимым запасным путём.
...
> [!CAUTION]
> Таблица ниже — исторический снимок зависимостей на 2026-07-06. Названия
> legacy effects/guards показывают требуемое поведение, но не действующие API.
> Актуальные зависимости определяются каталогом Mechanics, `moduleLock` и
> схемой Mechanics IR по ADR-084.
...
> [!CAUTION]
> JSON в этом разделе снят с исполнения и хранится только как точная
> спецификация игровой семантики. Его нельзя копировать; реализация обязана
> сохранить результаты и порядок правил через Game Intents и Mechanics IR.

Все 15 действий приведены в исторической форме, чтобы сохранить точную
семантику миграции. Их legacy guards/effects не копируются; числа, входные
условия, порядок вычислений и результаты сохраняются при переводе в Mechanics
IR. В старой форме служебные поля каждого действия были одинаковы:
...
| 2 | Изображения, созданные ИИ-генератором для проекта (в т.ч. локальным пайплайном генерации, вызываемым агентом) | Улучшение графики без художника | `{"type": "authored-in-repo"}`; условия инструмента/модели обязаны разрешать использование без атрибуции; инструмент и промт зафиксировать в `.desc.json` каталога `assets/` |
...
канала (`game-asset-channel-design.md` §3.5). Для бинарных форматов
(png/webp) агенту запрещён единственный режим — сочинять байты «из головы»
(в том числе рукописные base64-вставки): такой файл непроверяем в ревью и
почти всегда битый. Бинарный ассет появляется только как выход инструмента
— локального пайплайна генерации, скрипта или скачивания из источника
таблицы — с фиксацией происхождения по правилам выше и прогоном валидатора
канала. Замена графики позже — это замена файла при том же
