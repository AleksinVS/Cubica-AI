...
4. **Недетерминированное не входит в быстрый PR-гейт.** Live LLM, реальные платежи и внешние сети проверяются отдельно: replay, моки провайдера, тестовый контур или финальная приёмка.
...
Рабочие уровни определяют момент запуска, а не тип теста:

| Уровень | Назначение | Обязательное правило |
|---|---|---|
| Edit loop | Быстрая обратная связь при изменении | Классифицировать риск и запускать самый узкий прямой тест; ориентир — до двух минут, но это не жёсткий предел для build/E2E. |
| Task boundary | Доказательство завершённого изменения | Повторить только проверки изменённого поведения и затронутой границы; не повторять дорогой тест для неизменённого кода. |
| PR/CI required | Сохранение обязательных гарантий в CI | Все пять действующих статусов сохраняются; дешёвые шлюзы проходят до E2E. |
| Final/release | Приёмка этапа или выпуска | Выполняется полный релевантный контур, когда узких доказательств недостаточно или риск требует этого. |

Типы тестов используются внутри этих уровней:
...
| Performance | Время ответа, деградации editor/player/runtime | targeted scripts, later load tests | task/release contour |
...
Селектор affected (консервативный выбор затронутых проверок) сопоставляет
изменённые пути с проверками. Он fail-closed: при неизвестном соответствии,
общем контракте, schema/authorization/security изменении или ошибке анализа
выбирается полный резервный набор. На первом этапе в CI селектор наблюдательный:
он публикует рекомендуемый набор и длительность, но не сокращает обязательные
проверки.

Целевой локальный интерфейс включает affected, failed/last-failed, profile и E2E smoke.
Полные логи остаются в `.tmp/`; в передачу результата попадает только значимый
фрагмент. После падения повтор допускается только с новой гипотезой или выбором
last-failed. Trace, video, screenshots и прочие тяжёлые E2E-артефакты создаются
только для ошибки.

Для комплексного QA-пакета допустимо поручить дешёвой модели несколько простых
read-only проверок. Пакет обязан назвать точные команды, границу чтения,
критерии приёмки и короткий формат результата; основной агент сверяет доказательства
и принимает итог. Отдельный субагент ради одной известной детерминированной
команды не создаётся.

Команды одного QA-пакета выполняются последовательно, пока не доказана изоляция
портов, build output, временных каталогов, фикстур, переменных окружения и
другого изменяемого состояния. Полные наборы, сборки и E2E нельзя запускать
параллельно в одном workspace. Метка `serial` управляет только возможностями
конкретного runner и не считается межпроцессной блокировкой.
...
Оставить package-local runners, но добавить общий policy layer: root `verify:*` scripts, shared fixtures, replay harness, contract governance, отчётность и финальный/release-контур.
...
Все пять имён статусов из `.github/required-checks.md` сохраняются. Реализация
может устранять пересечение их работы, но не совокупное покрытие. AJV/schema,
contract, security и authorization проверки обязательны. Дешёвые шлюзы должны
заканчиваться до запуска E2E; устаревший прогон одного pull request отменяется.
Защита `main` остаётся отключённой по решению PM.

Полный резервный набор для неизвестного или критичного изменения включает:
...
- Все релевантные PR/CI required-проверки.
- Полный E2E в чистом окружении без reuse existing servers, когда пользовательская граница или риск выпуска требуют этого.
...
- **Live eval отдельно.** Запуски с реальным LLM provider идут перед релизом или в контролируемом тестовом контуре, с лимитом бюджета и отчётом о drift; отдельный nightly-прогон не создаётся.
