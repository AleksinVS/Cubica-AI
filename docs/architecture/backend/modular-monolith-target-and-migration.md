...
- каталог удалён 2026-08-06: его интерфейсные заготовки расходились с уже
  работающей более строгой транзакционной моделью `runtime-api`; история
  доступна в системе контроля версий, а не как параллельный образец.

Долгосрочно:

- не создавать отдельный deployable сервис до измеренной причины независимого
  масштабирования или развёртывания; извлечение начинать из действующих модулей
  и текущего OpenAPI-контракта `runtime-api`.
