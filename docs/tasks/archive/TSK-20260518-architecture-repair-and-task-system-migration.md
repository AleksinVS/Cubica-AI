...
Ядро задачи — миграция системы задач на ADR-031 — выполнено: ADR-031 принят, доска
`NEXT_STEPS.md` + активные `TSK-*` в работе. Восстановление зелёного canonical slice и
синхронизация документации перекрыты более новыми remediation-задачами
[[TSK-20260520-project-review-remediation]] и блоком `TSK-20260630-*`. Остаточной
самостоятельной работы нет; задача закрыта как поглощённая.
...
2. **Potentially valid tool/agent files. Classify, then keep or move.**
   ...
   - `.gemini/`;
   - `.geminiignore`;
   - `PROJECT_WORKFLOW_CONFIG.json` — классификация завершена ADR-068: файл архивирован и удален как устаревшая точка входа;
   - `.gitmodules`.
...
- `docs/tasks/archive/TSK-20260518-architecture-repair-and-task-system-migration.md` — этот рабочий план.
...
- `docs/tasks/archive/TSK-20260518-json-schema-strict-validation.md` — follow-up по строгой JSON Schema validation.
- `docs/tasks/archive/TSK-20260518-workspace-project-references-cleanup.md` — follow-up по workspace/project references cleanup.
