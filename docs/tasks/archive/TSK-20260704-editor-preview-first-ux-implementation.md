...
implemented (2026-07-07) — Phases 0–9 (core) реализованы; концепты §9.7–9.8 вынесены в `TSK-20260707-editor-concept-surfaces`

Реализованы: Phase 0 (сверка), Phase 1 (контракты ядра: occurrences, риск-политика,
линзы, схема фикстур, schema-признаки отображения/декоративности), Phase 2
(кэш L1/L2/L3 + алгофикс компилятора 3.4×), Phase 3 (дерево «По экранам/По типам»
с occurrences + панель сущности по макету), Phase 4 (интерпретатор возвращённого
намерения + текстовый режим), Phase 5 (оси времени + фикстуры), Phase 6 (создание/
удаление/переименование/создание вида через мультидок-конвейер с approval), Phase 7
(очередь интентов с оптимистичной конкуренцией), Phase 8 (диагностический поток +
last-valid снимок), Phase 9 core (регион-снимок как optional adapter capability +
секция «Ассеты»). Инфраструктурно: срез A (OOM-безопасный prod-режим e2e) и B
(декомпозиция EditorWorkspace, модуляризация закрыта). Метрики: editor-engine 187
тестов, editor-web 232, e2e 8/8 (переподтверждён независимо на каждом срезе).
Концепты §9.7–9.8 (история версий, Telegram-просмотрщик, viewport) —
`TSK-20260707-editor-concept-surfaces`. Открытые вопросы владельцу и follow-ups —
в Handoff Log (запись Phase 10).
...
- 2026-07-07 (Phase 8.1 — вкладка «Проверки» + навигация + быстрые исправления,
  готово): `checks-helpers.ts` (чистая game-agnostic агрегация ВСЕХ диагностических
  потоков — projection + schema/syntax/semantic/reverse/ai/workflow + plugin — в
  дедуплицированный, сгруппированный по серьёзности список; указатель→сущность
  обходом вверх; счётчики, бейджи фасета). Панель `checks-sidebar-panel.tsx`
  введена вкладкой левой activity-bar (правого сайдбара с вкладками Чат/Журнал/
  Проверки в коде нет — переиспользована сессионная область по инструкции).
  Навигация §9.6: клик → указатель→сущность через проекцию → handleEntityTreeSelectEntity
  (reveal в дереве + инспектор) → selectPointerNode на проблемном поле; невизуальные
  — дерево+панель. Быстрые исправления: детерминированный «Создать вид» для
  `entity-missing-view` (`buildAddViewFacetChangeSet` через единую точку);
  недетерминированные (висячая ссылка — нет билдера ретаргета одиночной ссылки;
  добавить обязательное поле — нужен резолв схемы; удалить неизвестный ключ —
  парсинг AJV) обоснованно оставлены под «Исправить агентом». «Исправить агентом»
  строит EditorPatchIntent (сообщение+контекст сущности+[code]) через очередь
  Phase 7 → единая точка apply. Счётчик статус-бара кликом открывает вкладку.
  Пустое состояние «Нет проблем». Гейты независимо переподтверждены: typecheck,
  unit 208 (+11), сборки в одиночку, e2e 8/8 (31.0с). Follow-ups 8.2: engine-билдер
  ретаргета одиночной ссылки; резолв схемы для add-required-field/remove-unknown-key;
  кросс-файловая навигация (сейчас в активном документе); `scenario-*` коды ещё не
  эмитятся (при появлении попадут во вкладку автоматически).
- 2026-07-07 (Phase 8.2 — last-valid снимок при сломанной компиляции, готово;
  Phase 8 закрыта): `previewUrl` = последний валидный снимок (ставится только при
  успешной компиляции, не гасится правкой из 5.1 и теперь не гасится при повторной
  сломанной компиляции — `clearPreparedPreview()` в ветке `!ready` только если
  `previewUrl===null`). Счётчик `editsSincePreview` (сброс при успехе, инкремент в
  softenPreviewForEdit после null-guard). Сломанная компиляция = блокирующие
  severity `error` (предупреждения НЕ блокируют, §9.6); свежесть `preview-blocked`
  (5.1) когерентна с плашкой. Плашка зона 3 (`derivePreviewBlockedPlate`/
  `formatPreviewBlockedMessage`): «Показана последняя рабочая версия — N правок
  назад. M ошибок блокируют обновление.» + «К первой ошибке» →
  `handleNavigateToFirstError` (открывает «Проверки» + navigate к первой
  error-диагностике из 8.1). Пустой экран запрещён: снимок остаётся под плашкой;
  нет снимка (первый компайл сломан) → «Предпросмотр ещё не собран — исправьте M
  ошибок» (не белый экран). Исправление ошибок → плашка исчезает, свежесть→
  актуален, авто-применение обновляет preview. timestamp не рендерится (макет
  показывает только N+M). Поведение 5.1 не изменено. Гейты независимо
...
- 2026-07-07 (Phase 9.2 — секция «Ассеты» + asset-reference виджет, готово;
  субагент завис до отчёта — верификацию довёл оркестратор): серверный
  `editor-asset-store.ts` (единый читатель/писатель ассетов;
  `listGameAssets`/`writeGameAsset`) + маршрут `/api/editor/assets` (GET листинг
  с типом по расширению/размером/счётчиком использований/флагом сироты; POST
  загрузка base64 в session worktree assets tree, коммит на Save; content-подмаршрут
  для превью). Счётчик использований — по конвенции asset-reference (строковое
  поле сущности = путь ассета; game-agnostic, не хардкод). Панель
  `asset-library-panel.tsx` (секция левого сайдбара: сетка превью, поиск, фильтр
  по типу, счётчик «используется в N местах», сироты). Диагностика `asset-orphan`
  (info, реестр §4) подключена в «Проверки» (checks-helpers, бейдж «ассет»). Виджет
  asset-reference в инспекторе (поле-ссылка): «выбрать» (пикер библиотеки) /
  «загрузить» / «сгенерировать». Правка ссылки — через существующий property-edit.
  **Генерация НЕ реализована**: кнопка намеренно неактивна (media-gen backend не
  подключён — documented follow-up; provenance `generation.prompt` — метаданные
  §9.4, pipeline не строился; агент не генерирует по своей инициативе). Гейты
  независимо переподтверждены оркестратором: typecheck, unit 232 (+5), сборки в
  одиночку, e2e 8/8 (29.6с). game-хардкода нет, e2e-селекторы целы. Следующее:
  концепты (§9.7–9.8) → под-TSK, затем Phase 10 closeout.
- 2026-07-07 (Phase 10 — CLOSEOUT): программа preview-first UX редактора
  реализована (Phases 0–9 core). Сводка по срезам — записи выше; всё в
  `origin/main`. Итоговые метрики: editor-engine 187 юнит-тестов, editor-web 232,
  контрактные Ajv-тесты (state-fixture, adapter), полный e2e 8/8 (независимо
  переподтверждался на КАЖДОМ срезе OOM-безопасным путём). ADR-057 реализован без
  изменения целевой архитектуры; PROJECT_ARCHITECTURE §ADR-057 обновлён с
  «запланирована» на «реализована». Концепты §9.7–9.8 → `TSK-20260707-editor-concept-surfaces`.

  **E2E-набор spec §6 — статус:** новые потоки Phase 3–9 (создание сущности,
  вкладка «Проверки», текстовый режим, фикстуры, удаление/rename) покрыты
  ОБШИРНЫМИ юнит/контракт-тестами на реальном ядре (editor-engine 187 +
  editor-web 232), плюс стабильный интерактивный baseline e2e 8/8 (session/
  preview/rollback/inspect/plugin + player-пути). Расширение интерактивного
  e2e-набора на новые потоки (spec §6 сценарии: drag-create, region→prompt→
  summary, tree-grouping switch, broken-compile→last-valid) — follow-up,
  гейтится нагрузкой хоста (задокументированный средовой блокер
  `docs/reviews/2026-07-05-...`: конкурентная сборка editor-web OOM-killed;
  интерактивные preview-тесты чувствительны к CPU-starvation). На менее
  нагруженной машине (≥8 ядер) добавить эти сценарии.

  **Открытые вопросы ВЛАДЕЛЬЦУ (не блокируют реализацию, требуют продуктового
  решения):**
  1. **Политика apply-гейта при пред-существующих ошибках** (Phase 6.2a):
     apply-гейт валидирует ВЕСЬ after-документ, поэтому создание/операции
     блокируются, если в целевом манифесте уже есть blocking-ошибки (antarctica:
     71 `cards` без `_label`). Консистентно со всем текущим apply, НЕ регресс.
     Смягчение до «нет НОВЫХ ошибок» — платформенная политика, затрагивает все
     операции. Решение владельца.
  2. **Языковая политика UI редактора** (Phase 3.b.2+): макет русскоязычный,
     chrome редактора английский; сейчас когерентно смешано (англ. chrome +
     рус. доменные подписи из макета). Нужна явная политика, чтобы не дрейфовать.

  **Follow-ups (зарегистрированы, не блокируют closeout):** перетаскивание
  прототипа на канвас (hit-test); мультидок для agent/returned-intent путей;
  provider-abort интента (AbortSignal); re-approval для «применить всё равно» на
  dangerous stale; приглушение старых событий трассы (per-event хеш манифеста);
  конверты графа/деревьев (кэш L2); проекция не захватывает именованные UI-
  прототипы; кросс-документная запись из панели/single-reference retarget билдер;
  schema-резолв для add-required-field/remove-unknown-key быстрых фиксов;
  мультивыбор; backend генерации ассетов; подключение `previewRendererAdapterRef`
  к same-origin рендереру (регион-снимки заработают с Telegram-просмотрщиком
  `TSK-20260707`); Model-2 полная кросс-reload атомарность мультидок-apply.
