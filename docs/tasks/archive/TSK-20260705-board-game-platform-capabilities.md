...
Status note: предметные требования ADR-058 сохраняются, но этот исполнительный
план заменён принятым контрактом ADR-084. Реализация через отдельные
`effects[]`, guards, runtime JsonLogic и handlers больше не продолжается.
Оставшиеся игровые возможности активируются новыми вертикальными срезами и
выражаются как Game Intent → типизированный Mechanics IR.

> [!IMPORTANT]
> Этот файл хранит историю доставки и требования к поведению пошаговых игр.
> Разделы Target State, Scope, Execution Plan, Acceptance и старые записи
> Handoff Log **не являются инструкцией по реализации**: запрещено возвращать
> `deterministic.effects[]`, `metric.set`, `metric.transfer`, `when`, legacy
> guards/handlers или полный `json-logic-js` в серверный runtime. Перед новым
> срезом требования переводятся в актуальные Game Intents, типизированные
> выражения, структурные узлы и команды Mechanics IR по ADR-084.
...
- `docs/architecture/adrs/084-typed-transactional-mechanics-ir.md` (Accepted) и
  `docs/architecture/runtime-mechanics-language.md` — единственный действующий
  контракт исполнения механик.
...
- `docs/architecture/board-game-platform-design.md` — детальный дизайн и инвентаризация правил.
  Он нормативен для предметного поведения, но его legacy-примеры эффектов не
  копируются в новые манифесты.
...
Ниже зафиксирован исторический снимок до полной миграции ADR-084; он не
описывает текущий runtime.
