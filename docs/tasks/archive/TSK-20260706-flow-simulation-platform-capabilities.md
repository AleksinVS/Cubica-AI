...
Status note: параметры действий и Phaser-канал сохраняются как принятые
возможности, но этот исполнительный план runtime-эффектов заменён ADR-084.
Оставшаяся работа активируется новым игровым срезом на текущем контракте.

> [!IMPORTANT]
> Разделы про `random.seed` effect, `metric.set`, `when`, runtime guards,
> JsonLogic context builder и старые handlers являются историей исходного
> плана. Они не должны быть реализованы или восстановлены. Текущий серверный
> путь — Game Intent → типизированный Mechanics IR; ограниченный JsonLogic
> остаётся только в player-facing computed metrics.
...
- `docs/architecture/flow-simulation-platform-design.md` — детальный дизайн.
  Для нового среза из §4.0 обязательны только сохраняющиеся params/Phaser/UI
  контракты; runtime effect-часть историческая. Игровое поведение §6 служит
  входом перепривязки, а форму исполнения задаёт ADR-084.
- `docs/architecture/board-game-platform-design.md` §4.0–4.1 — нормативный PRNG
  (владелец — параллельный трек); legacy JsonLogic-контекст исполнения заменён
  типизированными выражениями Mechanics IR.
...
Ниже приведён исторический снимок на момент создания задачи, а не описание
текущего runtime.
