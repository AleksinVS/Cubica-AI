...
DR-20 реализован 2026-08-31 как локальная бесконтентная диагностика будущего
`gateway_error`. Evaluator до cleanup показывает только существующий
`last_error_code`, принятый закрытым classifier worker и точно связанный с
одним hard-stopped сценарием; неизвестное значение подавляется. Постоянный
report, JSON Schema, PostgreSQL, metric, retry, provider/model и код завершения
не изменены. Новый внешний вызов по-прежнему требует отдельного решения PM.
...
DR-20 добавил для будущего отдельно разрешённого окна только локальный
allowlisted код до cleanup. Публичный отчёт, retry-политика и provider не
менялись; сама диагностика не разрешает внешний вызов.
...
8. проверить локальные DR-15/DR-20 диагностики: только allowlisted stage или
   gateway-код в
   существующем `last_error_code`, без raw provider payload, candidate,
   идентификаторов или содержимого; зафиксировать stage или код до cleanup;
