...
серверный bare Git и эксплуатационное расписание очистки. PM дал отдельное
одобрение 2026-08-11, но официальный договорный ворот provider не пройден:
Coding Plan нельзя использовать из Cubica как собственного приложения без
письменного соглашения с Z.AI. Источник:
...
4. получить письменное разрешение Z.AI для текущего Coding Plan adapter либо
   согласовать и реализовать стандартный API/другой provider с отдельным
   credential, стоимостью и ревизией политики внешней обработки; затем задать
   серверный путь `CUBICA_PRODUCT_CONTEXT_SHADOW_KNOWLEDGE_REPOSITORY` к bare Git;
...
6. назначить одного разработчика владельцем одной игры: текущий локальный
   Portal содержит пользователей и игру, но не содержит связи `developed_by`;
   затем зафиксировать ограниченный объём уже согласованного реального прогона.

До закрытия пунктов 4 и 6 реальные тексты не отправляются. Сохранённый в `.env`
Coding Plan key не использовался в этом операторском проходе.
