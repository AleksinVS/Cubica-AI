...
| Private invite network сохраняет actor boundary и reconnect | S10 (GSR-050) | optional `local\|private-invite`, immutable session-owned participants, creation-only invite links, hash-only secrets, SSE `{stateVersion,lastEventSequence}` + authenticated full GET/resync; те же intents | два браузерных контекста и reconnect state | spoofing/privacy, stale version, reconnect/resync | реализация существует; production two-browser E2E и primary visual acceptance pending |
...
| Private invite network | S10 (GSR-050) | S8 и локальный S9 приняты; game manifest не меняется; реализация v1 существует в узкой границе ADR-059 | network UI существует; primary visual acceptance pending | runtime 395 pass / 2 skipped / 0 fail, contracts-session 16/16, player-web 332/332, typechecks/API contract gate green, PostgreSQL restart 1/1; two-browser E2E pending | в исполнении до финальных production two-browser E2E и primary visual acceptance |
