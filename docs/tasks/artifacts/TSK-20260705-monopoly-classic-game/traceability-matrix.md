...
| ИИ получает только проекцию и доступные действия, а не состояние движка | S9 | S8-контракт принят; agent seat/fallback declarations; agent action choice → обычный dispatcher | состояние ИИ и fallback | adversarial mock, invalid choice, ordered safe fallback, pause/human takeover при недоступном Agent Runtime | запланировано; S9 ещё не реализован |
...
| ИИ-места | S9 | S8-контракт и ADR-060 приняты; отдельную ветку runtime создавать нельзя | AI seat UI не активируется | adversarial projection/fallback checks | запланировано: следующий срез |
