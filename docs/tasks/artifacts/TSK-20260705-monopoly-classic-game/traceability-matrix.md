...
| Private invite network | S10 | S8 и локальный S9 приняты; game manifest не меняется, network lifecycle остаётся отдельной границей | network UI не активируется | platform task + PostgreSQL/concurrency/browser checks | следующий срез |
