...
| `deck.shuffle` | Schema `:1737`; `deckEffects.ts:101` | Mock, 2 | Каноническая выборка источника, секретный порядок, серверное перемешивание | A | Версионированный secret-safe deck module с typed source, объявленным назначением случайности и лимитами; old entry point удаляется. P2. |
