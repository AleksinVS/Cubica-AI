...
| Recorded playthrough | preview action events + runtime snapshots | browser state plus `.tmp/editor-playthroughs/` trace documents | runtime-api restores exact preview snapshot first; sparse snapshot replay remains compatible follow-up | nonlinear session can move backward; rollback/new play keeps one linear path |
...
| Preview rollback | runtime-api integration test for `/sessions/:id/preview-restore`; editor-web preview message/trace tests; player-web/editor-web typecheck; browser e2e for full iframe rollback and explicit trace controls | Rollback mutates authoring JSON, works for production sessions, branches future trace, fails to truncate persisted trace, hides the current marker, or desynchronizes player-web from runtime-api. |
