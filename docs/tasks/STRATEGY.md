...
JSON manifest остаётся авторитетным объявлением исполнимой логики: он фиксирует
игровые намерения (`Game Intent` — команды в терминах игры), Mechanics IR
(типизированные исполняемые планы), версии модулей и, при необходимости, hash,
schema-defined контракт и права изолированного runtime-расширения
(исполняемого модуля, принадлежащего одной игре).
Исполняемый код допускается в bundle конкретной игры, когда декларативных и
общих платформенных механизмов недостаточно; он не переносит предметную ветку в
ядро и не получает прямого доступа к платформе. JSON Schema валидирует
публичные структуры и контракты. Ручные проверки допустимы только как отдельный
compiler/semantic validation layer, а не как замена схеме.

### 4.3. Platform Purity

Платформенные слои не должны содержать hardcoded game IDs или условия для
конкретной игры. Игра-специфичная логика живёт в game package, manifest,
declarative plugin layer или изолированном runtime-расширении игры. Gameplay
Slice Records документируют ограниченный сценарий доставки, но не являются
runtime source of truth.

Публичный язык игрового движка следует проектировать универсальным уже от
первого доказанного игрового сценария. Общий runtime предоставляет нейтральные
операции выбора, вычисления, композиции, планирования и изменения состояния;
предметные глаголы остаются в игре или раскрываются authoring-компилятором в
эти операции. Запрет лишних абстракций действует для внутреннего служебного
кода без потребителя, но не оправдывает узкий публичный игровой контракт.
...
- включает только востребованные срезом доработки, но проектирует механизмы игрового движка универсальными и комбинируемыми: предметное правило игры собирается из общих операций, а не переносится в ядро как узкий именованный эффект;
...
- заменяет узкие предметные операции игрового движка общими декларативными механизмами выбора, вычисления, композиции, планирования и изменения состояния;
...
- добавляет внутренний технический слой платформы без текущего потребителя и без связи с публичным языком игрового движка;
