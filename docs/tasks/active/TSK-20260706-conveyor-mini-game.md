...
Status note: задача требует перепривязки к ADR-084 до реализации. Игровой
сценарий и Phaser-канал сохраняются, но старые зависимости на `metric.set`,
`when`, runtime guards и JsonLogic удалены из платформы.

> [!IMPORTANT]
> Полные JSON действий и исполнительные указания старого flow-design нельзя
> копировать. Они описывают требуемое поведение «Мини-конвейера», а не текущий
> контракт. Реализация должна заново скомпилировать это поведение в Game
> Intents и типизированный Mechanics IR; JsonLogic допускается здесь только в
> player-facing computed metrics, но не как серверный guard/effect executor.
...
- `docs/architecture/flow-simulation-platform-design.md` — **§6 нормативен
  для игрового поведения, контента, экранов, раскладки плагина и поведения
  сцены**. Legacy JSON действий из §4/§6 заменён ADR-084 и не является
  исполнимым образцом. Действующие формы механик задают ADR-084 и
  `runtime-mechanics-language.md`.
...
1. Готовы `paramsSchema` + `params`, Phaser-хост + `simulationSurface` +
   `phaserSceneFactory`, `createSeededRandom` и типизированная Mechanics-
   операция выдачи зерна раунда; legacy effect `random.seed` не требуется.
2. Типизированные Mechanics IR выражения, assertions и команды должны покрыть
   проверку результатов и расчёт очков; legacy `metric.set`/`when` не являются
   допустимой зависимостью.
...
- 2026-07-15: исполнительная форма эффектов из исходного дизайна заменена
  ADR-084. Задача остаётся заблокированной до перепривязки полного игрового
  сценария к Game Intents и Mechanics IR; старые JSON-примеры не исполняются.
