...
> [!IMPORTANT]
> После ADR-084 задача требует перепривязки механик. Баланс, контент, Phaser-
> сцена и приёмочные числа исходного дизайна сохраняются, но полные JSON
> `handlerType`/guards/`effects[]`/`when` и runtime JsonLogic больше не являются
> контрактом и не копируются. Серверная логика реализуется через Game Intents,
> типизированные Mechanics IR выражения и команды.
...
- `docs/architecture/rail-tycoon-mini-design.md` — **единственная нормативная
  спецификация поведения игры**: §4 механика/баланс, §5–6 предметные данные,
  §7 UI-манифест, §8 плагин/сцена, §9 тесты и приёмочные числа, §10 закрытые
  решения. Исполнительная форма §5–6 заменена ADR-084; приоритет у актуальных
  схем и `runtime-mechanics-language.md`, а не у legacy JSON дизайна.
...
|---|---|---|---|
| D1 | Закреплённый модуль воспроизводимой случайности и типизированная IR-операция броска | ADR-084 и текущий Mechanics catalog | module lock и нейтральные тесты алгоритма |
| D2 | Типизированные выражения, assertions и команды расчёта экономики | ADR-084 и текущий Mechanics catalog | актуальная схема Mechanics IR и нейтральные тесты |
...
- [ ] `game.manifest.json`: `meta`/`config`/`engine` по §5 дизайна (образец
      ...
      сохранить семантику всех 15 действий §6, но выразить их актуальными Game
      Intents и Mechanics IR; legacy JSON не копировать.
