...
Первый implementation slice должен дать поведенчески идентичный output. Допустимая стратегия: обернуть текущий runtime manifest в верхнеуровневый authoring definition и получить byte-equivalent generated manifest после компиляции.
...
- Next implementation step: create `games/antarctica/authoring/**` and prove byte-equivalent compiler output for the game manifest before extracting smaller prototypes.
