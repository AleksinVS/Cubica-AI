...
- универсальный язык механик нужно развивать от первого реального игрового
  сценария, не закрепляя предметные эффекты; риск контролируется схемой,
  лимитами выполнения и нейтральными фикстурами, а не ожиданием второго случая;
...
- узкие публичные контракты, которые потребуют массовой миграции игр;
...
- Мы **не** разрешаем произвольный код, неограниченные циклы, скрытый доступ к
  окружению или предметные операции в общем runtime. Универсальный
  декларативный язык и selector framework развиваются по ADR-083 от потребностей
  текущих игровых срезов.
