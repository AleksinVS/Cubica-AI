...
**Зарезервированный ключ `_collapse`.** Каталог, у которого в `.desc.json` объявлено `"_collapse": true`, попадает в `PROJECT_STRUCTURE.yaml` одной строкой `directory...` без содержимого. Это обязательный режим для архивов (`archive/`, `docs/tasks/archive/` и любых будущих архивов): карта структуры описывает текущий репозиторий, а не историю, поэтому завершённые `TSK-*` файлы и исторические снимки не перечисляются в ней пофайлово. Пофайловые описания внутри архивного `.desc.json` при этом сохраняются там, где их требуют governance-проверки (например, описания задач в `docs/tasks/archive/.desc.json`), — они остаются метаданными и в карту не публикуются.

Пример архивного `.desc.json`:
```json
{
  "_collapse": true,
  ".": "Архив задач: завершённые TSK-файлы и старая иерархия планирования."
}
```
...
- **Сворачивание архивов:** Каталоги с `"_collapse": true` в `.desc.json` выводятся одной строкой без содержимого (см. описание ключа `_collapse` выше).
- **Источники данных:** 
...
function buildTree(dirPath, depth = 0) {
  ...
  // Архивные каталоги ("_collapse": true в .desc.json) выводятся одной
  // строкой без содержимого: карта описывает текущий репозиторий, а не историю.
