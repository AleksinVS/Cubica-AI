...
воротами полноты `coverage`, слоёная реализация, приёмка через `ui-compare`. Одноразовый предшественник —
`scripts/debug/visual-diff-journal.mjs` (захардкожен под экран журнала;
для новых задач использовать инструмент навыка).
