...
- [Обзор](#обзор)
- [0. Подготовка мок-данных и фикстур](#0-подготовка-мок-данных-и-фикстур)
- [1. Подготовка автоматизированной сверки (Visual Diff)](#1-подготовка-автоматизированной-сверки-visual-diff)
- [2. Анализ DOM-структуры](#2-анализ-dom-структуры)
- [3. Анализ CSS: поэтапное устранение расхождений](#3-анализ-css-поэтапное-устранение-расхождений)
- [4. Применение фиксов через CSS overrides](#4-применение-фиксов-через-css-overrides)
- [5. Итеративная проверка](#5-итеративная-проверка)
- [6. Критерий успеха](#6-критерий-успеха)
- [Диагностические скрипты](#диагностические-скрипты)
- [7. Проверка кликабельности и интерактивности](#7-проверка-кликабельности-и-интерактивности)
- [8. Продвижение до целевого состояния (State Reaching)](#8-продвижение-до-целевого-состояния-state-reaching)
- [9. Дополнительные рекомендации по практике миграции](#9-дополнительные-рекомендации-по-практике-миграции)
- [10. Находки 2026-07: ограничения vision-моделей и сравнение по регионам](#10-находки-2026-07-ограничения-vision-моделей-и-сравнение-по-регионам)
- [Чек-лист](#чек-лист)
...
**Автоматизация для ИИ-агентов:** рабочий цикл этой методики (скриншот → пиксельное сравнение → локализация → фикс) оформлен как навыки агента: `.claude/skills/ui-compare` (сравнение UI с образцом, включает готовый инструмент `ui-visual-tool.mjs`: capture / compare / crop / sample / to-png) и `.claude/skills/ui-from-mockup` (разработка UI по макету-изображению). Навыки используют эту методику как углублённый справочник.
...
- `includeAA` — учитывать ли антиалиасинг (сглаживание краёв текста).
  Уточнение 2026-07: рекомендуемое значение по умолчанию — `false`
  (антиалиасинг игнорируется): AA-шум от шрифтов маскирует реальные дефекты
  и приучает игнорировать diff. Включать `true` только осознанно, с записью
  причины в отчёте (см. раздел 10 и навык `ui-compare`).
...
## 10. Находки 2026-07: ограничения vision-моделей и сравнение по регионам

Блок добавлен по итогам анализа проблемы «агент упорно не видит очевидные
различия в UI» и создания навыков `ui-compare` / `ui-from-mockup`
(см. [Обзор](#обзор)). Содержит выводы и предложения, дополняющие разделы 1,
6 и 9.2.

### 10.1. Почему агент «не видит» очевидных различий

Это системные ограничения, а не небрежность конкретного агента:

1. **Vision-модель видит смысл, а не геометрию.** Она надёжно читает «синяя
   кнопка с текстом», но не отличает отступ 13px от 16px, `#0f4c75` от
   `#14507a` и сдвиг блока на 6px. Внешние эксперименты (2025–2026)
   подтверждают: LLM непригодны как движок поиска визуальных различий;
   инструменты visual regression используют их только для классификации
   различий, найденных детерминированным пиксельным сравнением.
2. **Скриншоты уменьшаются при попадании в контекст модели** — мелкие
   различия исчезают физически, до всякого анализа. Отсюда правило: детали
   смотреть только на кропах (вырезках) 1:1 или с увеличением, а не на
   полном кадре.
3. **Подтверждающее смещение**: агент, который сам писал код, ожидает
   правильный результат — и «видит» его, рационализируя различия
   («визуально совпадает»). Числа не рационализируются.
4. **Сравнение несравнимого** (разные данные, состояние экрана, кэш
   dev-сервера, DPR) — уже описано в разделах 0, 8, 9.4, 9.6; здесь
   упоминается как четвёртая типовая причина «слепоты».

Следствие — разделение ролей: **детерминированный код находит и локализует
различия; модель объясняет их причины и чинит**. Никогда наоборот.

### 10.2. Эффект разбавления: предложение к критерию успеха (раздел 6)

Обнаружено на практике: локальный дефект (пропавшая кнопка, закрашенный блок)
даёт ~25% различий в своей зоне, но лишь 2–3% по кадру 1920×1080 — и проходит
общий порог 5%. Именно так «очевидное различие» остаётся незамеченным при
формально пройденной проверке.

**Предложение (реализовано в `ui-compare`):** дополнить критерий успеха
раздела 6 вторым условием — помимо общего diff < 5%, **ни одна ячейка сетки
4×4 (или именованный регион) не превышает 10%**. Непройденные зоны обязаны
получить классификацию: дефект / шум рендеринга / намеренное задокументированное
отличие (аналог «трёх корзин» ADR-057, молчаливое игнорирование запрещено).

### 10.3. Два режима строгости по происхождению образца

- **pixel-parity** — образец является скриншотом работающего UI (draft,
  прежняя версия): применимы пиксельные ворота разделов 6 и 10.2.
- **style-parity** — образец сгенерирован (Midjourney и т.п.) или нарисован:
  пиксельный процент не применим; сравниваются дизайн-токены (замеренные
  цвета, типографика, структура и порядок зон). Согласуется с принципом
  «картинка — концепт, а не пиксельный эталон»
  (`docs/architecture/game-ui-mockup-flow.md`).

Режим фиксируется до начала работ — иначе агент будет гоняться за
недостижимым пиксельным совпадением с генеративной картинкой.

### 10.4. Сравнение по семантическим регионам (развитие раздела 9.2)

Раздел 9.2 предлагает зоны-полосы (header/cards/footer). Следующий шаг —
регионы, ограничивающие **конкретные визуальные элементы** (кнопка, карточка,
индикатор), с разбором в терминах элементов: «кнопка btn-move: 34% diff», а
не «ячейка r3c4». Источник границ — семантическая разметка образца:

- для макетов-изображений — `*.design.json` (ADR-016): `regions[].bounds` и
  `elements[].bounds`; разметку предлагает vision-модель, корректирует человек
  (поток `game-ui-mockup-flow.md`);
- для образцов-скриншотов работающего приложения — DOM самого приложения
  (`getBoundingClientRect` по селекторам), это точнее и бесплатно.

Важно: чем мельче размечены регионы, тем меньше эффект разбавления (10.2)
внутри самого региона. Крупная зона «весь контент» разбавляет дефект так же,
как полный кадр.

Двухуровневая схема для каждого элемента разметки (реализована командой
`compare-elements` инструмента навыка `ui-compare`):

1. **Геометрия («не там»)**: bounds элемента в разметке образца против
   фактического `getBoundingClientRect` соответствующего DOM-элемента.
   Числовой допуск ±2px (`--tolerance`).
2. **Внешний вид («не такой»)**: пиксельное сравнение кропа образца по bounds
   разметки с кропом реализации по фактическим bounds (с нормализацией
   размера). Так «элемент сдвинут» и «элемент выглядит иначе» перестают
   смешиваться в одном проценте.
3. **Перекрытие**: `elementFromPoint` в центре элемента; перекрытый
   интерактивный элемент — отказ, даже при идеальных пикселях (случай
   «кнопка видна, но не кликается», раздел 7).
4. **Отсутствие**: ненайденный селектор фиксируется как явный `MISSING`,
   а не растворяется в общем проценте.

Связка «регион → селектор»: необязательное поле `selector` у зоны/элемента в
`*.design.json` (схема ADR-016 объявляет `additionalProperties: true`,
дрейфа схемы нет) либо отдельная карта `--selectors map.json`
(`{"id": "css"}`), когда разметку менять нельзя. Долгосрочно селектор
выводится из дисциплины идентичности ADR-057 (UI-элемент → `id` игровой
сущности → data-атрибут в DOM рендерера).

Статус: сравнение по регионам из `design.json`, автокропы проблемных зон и
двухуровневая проверка `compare-elements` реализованы в инструменте навыка
`ui-compare` и проверены на эталонных страницах с пятью классами дефектов
(сдвиг, перекраска, перекрытие, отсутствие — `MISSING`, уход за пределы
вьюпорта — `OFFSCREEN`).

### 10.5. Перекрытия и слои

Элементы перекрывают друг друга (модальные окна, декоративные оверлеи, тени),
и кроп региона всегда содержит чужие пиксели. Правила:

1. **Вердикт всегда по плоскому изображению** — именно его видит игрок;
   отдельного «послойного» пиксельного критерия не существует. Слои — предмет
   диагностики: чей пиксель неправильный.
2. **Z-порядок фиксируется при разметке/инвентаризации** («что поверх чего»):
   из плоской картинки он восстановим только человеком/vision в момент
   разметки, а при реализации превращается в проверяемые требования
   (`z-index`, `pointer-events`) и в точки проверки кликабельности
   (`elementFromPoint` в зонах перекрытия — раздел 7).
3. **Изоляция элемента** (временное `visibility: hidden` оверлея через
   `page.evaluate`) — допустимый диагностический приём, чтобы понять, какой
   слой даёт расхождение; в образце-изображении слои не разделяются, поэтому
   изолированный кроп сравнивают только с пониманием этого ограничения.
4. **Тени и свечения выходят за bounds элемента**: различие в элементе даёт
   «ореол» diff в соседних регионах — при классификации зон учитывать зону
   влияния, а не только формальные границы.
5. **Анимации замораживать** перед скриншотом (CSS `animation: none`,
   `transition: none` или ожидание конца анимации) — иначе региональный diff
   нестабилен от прогона к прогону. Реализовано: команды `capture` и
   `compare-elements` инструмента замораживают анимации и ждут загрузки
   шрифтов (`document.fonts.ready`) по умолчанию; флаг `--no-freeze`
   отключает.

### 10.6. Инструментарий

Цикл автоматизирован навыками (см. [Обзор](#обзор)): `ui-compare` — команды
`to-png` / `capture` (со сценарием достижения состояния `--steps` и
заморозкой анимаций) / `compare` (общий % + сетка + именованные регионы из
`design.json` + автокропы «образец|реализация|diff» проблемных зон) /
`compare-elements` (двухуровневая проверка по элементам, раздел 10.4) /
`coverage` (числовая проверка полноты разметки: % площади вне зон + маска
непокрытых мест — защита от пропущенных элементов) / `crop` / `sample`;
флаг `--ci` даёт ненулевой код выхода при FAIL для интеграции в
`verify:*`-конвейеры. `ui-from-mockup` — инвентаризация макета до кода с
воротами полноты `coverage`, слоёная реализация, приёмка через `ui-compare`.
Удалённые одноразовые скрипты прошлых расследований не являются частью метода;
для новых задач используется только поддерживаемый инструмент навыка.
