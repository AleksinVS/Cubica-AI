...
`ui-compare` и проверены на эталонных страницах с четырьмя классами дефектов
(сдвиг, перекраска, перекрытие, отсутствие).
...
5. **Анимации замораживать** перед скриншотом (CSS `animation: none`,
   ...
   нестабилен от прогона к прогону.

### 10.6. Инструментарий

Цикл автоматизирован навыками (см. [Обзор](#обзор)): `ui-compare` — команды
`to-png` / `capture` / `compare` (общий % + сетка + именованные регионы из
...
`crop` / `sample`; `ui-from-mockup` — инвентаризация макета до кода, слоёная
реализация, приёмка через `ui-compare`. Одноразовый предшественник —
