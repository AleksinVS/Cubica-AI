...
**ВАЖНО:** Пиксельное сравнение через `pixelmatch` является **ОБЯЗАТЕЛЬНЫМ** шагом. Визуальная проверка "на глаз" или DOM-анализ без скриншотов недостаточны. Каждая итерация CSS-фиксов должна завершаться запуском скрипта `pixelmatch` и подтверждением достижения целевого процента.
...
**Обработка намеренных структурных отличий:** Если target должен отличаться от draft структурно (например, по требованию пользователя добавлены новые колонки или изменён layout), такие отличия должны быть задокументированы в ADR или задаче. В этом случае целевой показатель применяется к сопоставимым зонам (например, цветам карточек, шрифтам, отступам), а не к зонам с намеренными структурными изменениями.
...
- [ ] **Playwright скриншотит оба экрана в одинаковом состоянии, и `pixelmatch` запущен — сравнение ОБЯЗАТЕЛЬНО**
...
- [ ] **pixelmatch показывает < 5% различий (или < 3% для итераций без структурных изменений)**
