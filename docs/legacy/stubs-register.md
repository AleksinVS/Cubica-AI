...
| LEGACY-0018 | editor-engine | packages/editor-engine/src/index.ts | full project review 2026-06-27 | Модуль editor-engine разбит на связные файлы, index.ts — тонкий фасад ре-экспортов; остаётся декомпозиция компонента EditorWorkspace (~2500 строк), вынесена в отдельный срез (Phase 4) | Frontend Team | active | Публичный API сохранён; поведение не меняется |
