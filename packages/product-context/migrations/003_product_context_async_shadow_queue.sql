...
GRANT SELECT ON product_context_shadow.conversation_threads TO product_context_shadow_worker_owner;
GRANT SELECT ON product_context_shadow.conversation_messages TO product_context_shadow_worker_owner;
...
BEGIN
  ...
  FOR changed_run IN
    ...
    WHERE status = 'calling_model' AND lease_expires_at <= p_now
    RETURNING *
  LOOP
  ...
  FOR changed_run IN
    ...
    WHERE status IN ('pending', 'retry_wait', 'leased') AND retained_until <= p_now
    RETURNING *
  LOOP
    INSERT INTO product_context_shadow.shadow_metrics (
      metric_id, run_id, owner_ref, request_id, outcome, duration_ms, input_bytes,
  ...
  WHERE status = 'retry_wait' AND attempts >= p_max_attempts AND next_attempt_at <= p_now;
  ...
  FOR changed_run IN
    ...
    SET status = 'blocked', outcome_code = 'gateway_blocked', lease_token = NULL,
        lease_expires_at = NULL, completed_at = p_now,
        last_error_code = 'attempts_exhausted', updated_at = p_now
    WHERE status = 'leased' AND attempts >= p_max_attempts AND lease_expires_at <= p_now
    RETURNING *
  LOOP
    INSERT INTO product_context_shadow.shadow_metrics (
      metric_id, run_id, owner_ref, request_id, outcome, duration_ms, input_bytes,
  ...
  WHERE attempts < p_max_attempts AND retained_until > p_now AND (
    (status IN ('pending', 'retry_wait') AND next_attempt_at <= p_now) OR
  ...
  token := 'lease_' || md5(selected_run.run_id || ':' || (selected_run.attempts + 1)::text || ':' || p_now::text || ':' || random()::text);
  UPDATE product_context_shadow.shadow_runs
  ...
  SELECT EXISTS (
    ...
    WHERE thread.thread_ref = selected_run.thread_ref
      ...
      AND thread.status = 'active' AND thread.retained_until > p_now
  ) INTO thread_valid;
...
BEGIN
  ...
  WHERE run_id = p_run_id AND lease_token = p_lease_token
    AND status IN ('leased', 'calling_model');
  IF NOT FOUND THEN RETURN NULL; END IF;
  SELECT jsonb_build_object(
...
SET search_path = pg_catalog, product_context_shadow
AS $function$
DECLARE run_row product_context_shadow.shadow_runs%ROWTYPE;
BEGIN
  IF p_next_attempt_at IS NULL OR p_next_attempt_at <= p_now THEN
    RAISE EXCEPTION 'invalid retry schedule' USING ERRCODE = '22023';
  END IF;
  SELECT * INTO run_row FROM product_context_shadow.shadow_runs
  WHERE run_id = p_run_id AND lease_token = p_lease_token
    AND status IN ('leased', 'calling_model') FOR UPDATE;
...
BEGIN
  ...
  SELECT * INTO run_row FROM product_context_shadow.shadow_runs
  WHERE run_id = p_run_id AND lease_token = p_lease_token
    AND status = 'calling_model' FOR UPDATE;
  IF NOT FOUND THEN RETURN NULL; END IF;
  ...
  SELECT count(*)::integer,
         ...
         count(*) FILTER (WHERE message.retained_until <= p_now)::integer
  ...
  SELECT EXISTS (
           ...
           SELECT 1 FROM product_context_shadow.conversation_threads AS thread
           WHERE thread.thread_ref = run_row.thread_ref AND thread.owner_ref = run_row.owner_ref
             AND thread.retained_until <= p_now
         )
  INTO thread_valid, thread_expired;

  terminal_outcome := CASE
    WHEN run_row.retained_until <= p_now OR expired_count > 0 OR thread_expired THEN 'retention_expired'
...
$function$;
  ...
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_claim(integer,integer,timestamptz,text,text,text) FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_reread(text,text) FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_mark_calling(text,text,text,integer,timestamptz) FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_retry(text,text,text,timestamptz,integer,integer,integer,timestamptz) FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_terminal(text,text,text,text,text,integer,integer,integer,timestamptz) FROM PUBLIC';
  EXECUTE 'REVOKE ALL ON FUNCTION product_context_shadow.worker_complete(text,text,jsonb,text,integer,integer,integer,integer,timestamptz) FROM PUBLIC';
  ...
  EXECUTE 'GRANT EXECUTE ON FUNCTION product_context_shadow.worker_mark_calling(text,text,text,integer,timestamptz) TO product_context_shadow_worker';
