...
import { isEditorAgentProtocolUserMessage } from "@/lib/ag-ui-event-adapter";
import { drawingAgentMessage, toEditorUserMessage } from "./mvp-agent-message";

afterEach(() => vi.restoreAllMocks());

describe("drawing message transport", () => {
  it("keeps a text-only authoring context as a separate AG-UI part", () => {
    const message = toEditorUserMessage({ text: "Измени кнопку", context: "Контекст выбранного источника: {}" });
    expect(isEditorAgentProtocolUserMessage(message)).toBe(true);
    expect(message.content).toEqual([
      { type: "text", text: "Измени кнопку" },
      { type: "text", text: "Контекст выбранного источника: {}" }
    ]);
  });
  ...
  it("sends original image and normalized overlay separately with annotations as context", async () => {
    const lineTo = vi.fn(), fillText = vi.fn(), strokeText = vi.fn();
    vi.spyOn(HTMLCanvasElement.prototype, "getContext").mockReturnValue({ beginPath: vi.fn(), moveTo: vi.fn(), lineTo, stroke: vi.fn(), fillText, strokeText } as unknown as CanvasRenderingContext2D);
    ...
    expect(strokeText).toHaveBeenCalledWith("Играть", 400, 200);
