...
const invite = { credential: `ses_${"a".repeat(43)}` };
...
describe("private invite fragments", () => {
  ...
  it("rejects malformed and oversized session identifiers", () => {
    expect(parsePrivateInviteFragment("#invite?sessionId=x")).toBeNull();
    expect(parsePrivateInviteFragment(`#invite?sessionId=${"x".repeat(257)}&credential=ses_${"a".repeat(43)}`)).toBeNull();
  });
});
