...
/**
 * Portal launch-link resolver page.
 *
 * Players open links copied from the consultant portal. This page is a small
 * browser-side mediator: it validates the portal link with Strapi, lets the
 * backend decide whether the runtime session is new or existing, and then sends
 * the browser to the player URL returned by the backend.
 */
...
export default function LaunchPage() {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const result = await resolveLaunchSession({
          token: params.token,
