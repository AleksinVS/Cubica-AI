...
(async () => {
  ...
  // Also get the action button display
  const btn = await page.$('.leftsidebar-screen .cards-container > .s1-card:first-child button.action-button');
  if (btn) {
    const disp = await btn.evaluate(b => window.getComputedStyle(b).display);
    console.log('button display:', disp);
  } else {
