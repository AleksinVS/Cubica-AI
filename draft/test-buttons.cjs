...
(async () => {
  ...
  // Get element info for buttons and their parents
  const info = await page.evaluate(() => {
    ...
    const screen = document.querySelector('.leftsidebar-screen, .s1-screen');

    // Find what element is at the center of each button
  ...
  // Try clicking
  console.log('\nTrying to click journal button...');
  try {
