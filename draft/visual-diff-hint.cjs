...
async function captureTargetHint(page, sessionId, outputPath) {
  ...
  try {
    await page.waitForSelector('.antarctica-hint-screen', { timeout: 10000 });
  } catch {
    console.warn('Hint selector not found, capturing anyway');
  }
...
async function captureDraftHint(page, outputPath) {
  ...
  try {
    await page.waitForSelector('.antarctica-hint-screen, .hint-area', { timeout: 10000 });
  } catch {
