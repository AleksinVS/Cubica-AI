...
(async () => {
  ...
  const info = await page.evaluate(() => {
    const screen = document.querySelector('.journal-screen, .journal-container');
    if (!screen) return { error: 'journal screen not found', html: document.body.innerHTML.substring(0, 500) };
    return {
      className: screen.className,
      rect: screen.getBoundingClientRect(),
      childCount: screen.children.length,
      html: screen.outerHTML.substring(0, 2000),
    };
  });
