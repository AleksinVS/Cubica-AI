...
async function extractVar(page, url, label, selector, advanceToBoard = false) {
...
async function main() {
  ...
  const draftScore = await extractVar(page, DRAFT_URL, 'DRAFT', '.game-variables-container .game-variable--score');
  const draftNonScore = await extractVar(page, DRAFT_URL, 'DRAFT', '.game-variables-container .game-variable:not(.game-variable--score)');
  const targetScore = await extractVar(page, TARGET_URL, 'TARGET', '.topbar-variables-container .game-variable--score', true);
  const targetNonScore = await extractVar(page, TARGET_URL, 'TARGET', '.topbar-variables-container .game-variable:not(.game-variable--score)', true);
