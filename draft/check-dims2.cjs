...
async function main() {
  ...
  const draftInfo = await draftPage.evaluate(() => {
    const el = document.querySelector('.default-main-screen, .game-screen');
    if (!el) return { found: false, classes: document.body.innerHTML.substring(0,200) };
    const rect = el.getBoundingClientRect();
    const s = getComputedStyle(el);
    return { found: true, className: el.className, width: rect.width, height: rect.height, bgSize: s.backgroundSize, bgImage: s.backgroundImage };
  });

  const targetInfo = await targetPage.evaluate(() => {
    const el = document.querySelector('.topbar-screen-shell, .game-screen');
