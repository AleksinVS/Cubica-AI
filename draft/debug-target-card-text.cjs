...
(async () => {
  ...
  const sel = '.leftsidebar-screen .cards-container > .s1-card:first-child';
  const el = await page.$(sel);
  if (!el) { console.log('NOT FOUND'); await browser.close(); return; }

  // Get card inner HTML
  const html = await el.evaluate(n => n.innerHTML);
  console.log('Card HTML:', html.substring(0, 300));

  const textStyles = await el.evaluate(n => {
    const p = n.querySelector('.s1-card-text');
    if (!p) return null;
  ...
  console.log('Text styles:', JSON.stringify(textStyles, null, 2));
