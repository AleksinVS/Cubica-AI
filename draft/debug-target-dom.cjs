...
(async () => {
  ...
  // Extract the leftsidebar-screen inner HTML structure
  const match = html.match(/class="main-screen leftsidebar-screen"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/);
  if (match) {
    const inner = match[1].substring(0, 2000);
    console.log(inner);
  } else {
