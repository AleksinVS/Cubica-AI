...
async function main() {
  ...
  const draftInfo = await draftPage.evaluate(() => {
    ...
    return {
      width: s.width, height: s.height, display: s.display, justifyContent: s.justifyContent,
      alignItems: s.alignItems, gap: s.gap, padding: s.padding, margin: s.margin,
      flexDirection: s.flexDirection, rect: { x: r.x, y: r.y, w: r.width, h: r.height }
    };
  });
