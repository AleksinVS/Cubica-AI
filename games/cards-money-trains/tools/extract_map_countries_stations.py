...
на две ПО-РАЗНОМУ нарисованные группы:
  ...
  - 2 значка размером ровно вдвое меньше (~60 x 60 точек), состоящие из
    ...
    без зубцов с шуточными подписями "9¾" и "π" (пасхалки художника
...
кружка сохраняются отдельным списком `decorativeMarkers` со статусом
"предложено, не подтверждено человеком" — они не отбрасываются молча.
...
# декоративные пасхалки — они используют одну и ту же заливку, различить
...
# пасхалка — гладкий кружок без отверстия, то есть ОДНИМ подпутём.
...
# Две декоративные пасхалки были опознаны ВИЗУАЛЬНО (внешним агентом,
...
# ---------------------------------------------------------------------------
# Шаг 8. Отбор значков станционного цвета (23 шестерёнки + 2 пасхалки)
# ---------------------------------------------------------------------------

def select_station_color_icons(
    fills: list[dict[str, Any]]
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    """Разделить все значки станционного цвета на шестерёнки и пасхалки.
    ...
    контур и внутренняя дырка), а декоративная пасхалка — гладким кружком
...
def build_result() -> dict[str, Any]:
    ...
    # --- значки станционного цвета: шестерёнки (проверка) + пасхалки ---
    ...
    # --- декоративные пасхалки: не отбрасываются молча, помечены как
    #     предложенная, не подтверждённая человеком интерпретация ---
    ...
    for index, fill in enumerate(decorative_sorted, start=1):
        ...
        decorative_records.append(
            {
                "id": f"decorative-marker-{index:04d}",
                ...
                "interpretation": (
                    "декоративная пасхалка художника карты (гладкий кружок "
                    "без зубчатого обода и без отверстия — форма отличается "
                    "от настоящего значка станции), НЕ игровая станция"
                ),
    ...
    summary = {
        ...
        "decorativeMarkerCount": len(decorative_records),
    ...
    return {
        ...
        "warning": (
            ...
            "используется средой исполнения. Метка decorativeMarkers "
            "'reviewStatus: proposed' / 'confirmationStatus: unconfirmed' "
            "означает, что интерпретацию двух маленьких значков ('9¾' и "
            "'π' как пасхалки художника, а не станции) предложил внешний "
            "агент по внешнему виду вырезки карты, и она ожидает "
        ...
        "decorativeMarkers": decorative_records,
    }
...
def main() -> None:
    ...
    print(f"Декоративных значков: {result['summary']['decorativeMarkerCount']}")
