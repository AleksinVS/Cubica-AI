...
- Bounded player-facing delivery is no longer only a generic action catalog: `game.manifest.json` now includes `content.antarctica` for intro info `i0`, boards `1..6`, `7..12`, `13..18` and `19..24`, team-selection step `15`, plus infos `i7`, `i8`, `i9`, `i10` and `i11`, the content endpoint projects it directly, and `player-web` resolves those scenes from the current session snapshot with a safe fallback for unmigrated steps.
