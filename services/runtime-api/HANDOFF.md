...
- **S1 info-screen `activeInfoId` projection (2026-04-12):** Все early S1 transitions (`opening.card.3/9/18/22/31/3902/48/49/60.advance`) теперь корректно устанавливают `timeline.activeInfoId` в manifest deterministic stateUpdate, что обеспечивает правильный рендеринг info-экранов вместо stale `i0`.
