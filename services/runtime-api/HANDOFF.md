...
- team-selection mechanics for step `15` are implemented, and the post-confirm mainline path now reaches `stepIndex = 32`; cards `61..66` and later gameplay slices are still unreached in canonical runtime;
...
3. Следующий Antarctica gameplay slice - mainline `stepIndex = 32` с cards `61..66` после уже внедрённых `GSR-020`, `GSR-021`, `GSR-022`, `GSR-023`, `GSR-025`, `GSR-026` и `GSR-027` под архитектурными ограничениями `ADR-024`.
