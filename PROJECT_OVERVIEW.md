...
> Текущее состояние: канонический канал доставки — `apps/player-web` с plugin-архитектурой (ADR-037/039/055); описанный ниже слоистый SDK — историческое target-направление, судьба которого решается отдельно (LEGACY-0031).
...
- Слоистая SDK-стратегия (`SDK/core`/`SDK/shared`/`SDK/react-sdk`/`SDK/viewers`) — историческое target-направление: `react-sdk` и `shared` выведены из workspaces как неиспользуемые, `viewers` не реализован (LEGACY-0005/0011/0021). Решение о судьбе направления (Superseded или возрождение) зафиксировано как долг LEGACY-0031.
