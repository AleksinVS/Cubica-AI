...
- **Проектная система знаний** — ADR-082 вводит `docs/knowledge/` как канон смыслового синтеза для агентов поверх специализированных источников истины. Markdown и конфигурация проверяются JSON Schema; wiki-граф, FTS5 и векторный путь остаются локальными пересобираемыми проекциями. Явный гибридный запрос и сравнительная оценка доступны сразу, а режим по умолчанию выбирается при приёмке первой версии по измеримому качеству с FTS как начальным резервом. Минимальная AST-проекция импортов добавляет фактическую карту зависимостей кода, но не подменяет смысловые связи вики. Существующая генерация `PROJECT_STRUCTURE.yaml` не заменяется.
