...
- ADR-050 - accepted source для извлечения и повышения authoring-прототипов: повторяющиеся game/UI authoring-элементы сначала выносятся в локальные game-level prototypes, проверенные локальные прототипы могут вручную повышаться до platform-level catalog, а AI-assisted designer только предлагает `EditorChangeSet` и не применяет изменения напрямую.
...
Основные положения:
  ...
  - **Authoring prototype extraction and promotion (ADR-050, Accepted)**: локальные прототипы игры являются первым уровнем дедупликации authoring-структур; платформенные прототипы появляются только через ручное повышение после проверки универсальности, версии, примеров и validation gates.
