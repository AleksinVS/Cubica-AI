...
- `games/antarctica/game.manifest.json` теперь содержит канонический `content.antarctica` block для bounded player-facing delivery slice: info scenes `i0`, `i7` и `i8`, boards `opening.board.1_6` и `opening.board.7_12`, а также card catalog `1..12` с явными `selectActionId` и `advanceActionId` для go-card `3` и `9`.
...
- `apps/player-web` начал использовать existing session snapshot (`timeline`, `activeInfoId`, `selectedCardId`, card flags) для current-step rendering: стартовый info `i0`, boards `1..6` и `7..12`, а также info `i7` и `i8` больше не требуют global action catalog как основной UX.
...
4. Первый bounded player-facing delivery slice уже закрыт для `i0 -> board 1..6 -> i7`, а этот bounded extension добавляет `board 7..12 -> i8`; следующий этап должен расширить current-step rendering дальше по opening flow, сохраняя manifest-driven content projection и fallback для ещё не перенесённых step-ов.
