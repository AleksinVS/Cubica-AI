...
4. Продолжать manifest-driven migration небольшими bounded slices: после закрытия `GSR-026` ближайший открытый slice теперь начинается с mainline boundary `stepIndex = 30`.
5. Переход `first board -> i7 -> second board 7..12 -> i8 -> board 13..18 -> i9 -> step 15 -> i10 -> board 19..24 -> i11 -> board 25..30 -> i12 -> board 31..36 -> i13 -> board 37..42 -> i14 -> i14_2 -> board 43..48 -> i15 -> board 49..54 -> i16` уже покрыт на manifest boundary level, включая bounded line switch на loss line `i34 -> i34_2 -> i21`, locked/unlocked go-card `39`, entry-time alt `3902`, explicit public-communication board progression и trusted-messengers board progression. Следующая естественная точка входа теперь - step `30` с cards `55..60`.
