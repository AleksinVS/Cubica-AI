...
- `games/antarctica/game.manifest.json` теперь содержит канонический `content.antarctica` block для bounded player-facing delivery slice: info scenes `i0`, `i7`, `i8`, `i9`, `i10`, `i11` и `i12`, boards `opening.board.1_6`, `opening.board.7_12`, `opening.board.13_18`, `opening.board.19_24` и `opening.board.25_30`, team-selection step `15`, а также card catalog `1..30` с явными `selectActionId` и `advanceActionId` для go-card `3`, `9`, `18`, `22` и `23`.
...
- `apps/player-web` начал использовать existing session snapshot (`timeline`, `activeInfoId`, `selectedCardId`, card flags, team flags, teamSelection state) для current-step rendering: стартовый info `i0`, boards `1..6`, `7..12`, `13..18`, `19..24` и `25..30`, team-selection step `15`, а также info `i7`, `i8`, `i9`, `i10`, `i11` и `i12` больше не требуют global action catalog как основной UX.
...
4. Первый bounded player-facing delivery slice уже закрыт для `i0 -> board 1..6 -> i7`, а последующие bounded extensions добавили `board 7..12 -> i8`, `board 13..18 -> i9`, `step 15 team-selection`, `i10`, `board 19..24`, `i11`, `board 25..30` и `i12`; следующий этап должен расширить current-step rendering дальше по opening flow, сохраняя manifest-driven content projection и fallback для ещё не перенесённых step-ов.
