...
- `games/antarctica/game.manifest.json` теперь содержит канонический `content.antarctica` block для bounded player-facing delivery slice: info scenes `i0`, `i7`, `i8` и `i9`, boards `opening.board.1_6`, `opening.board.7_12` и `opening.board.13_18`, а также card catalog `1..18` с явными `selectActionId` и `advanceActionId` для go-card `3`, `9` и `18`.
...
- `apps/player-web` начал использовать existing session snapshot (`timeline`, `activeInfoId`, `selectedCardId`, card flags) для current-step rendering: стартовый info `i0`, boards `1..6`, `7..12` и `13..18`, а также info `i7`, `i8` и `i9` больше не требуют global action catalog как основной UX.
...
4. Первый bounded player-facing delivery slice уже закрыт для `i0 -> board 1..6 -> i7`, а последующие bounded extensions добавили `board 7..12 -> i8` и `board 13..18 -> i9`; следующий этап должен расширить current-step rendering дальше по opening flow, сохраняя manifest-driven content projection и fallback для ещё не перенесённых step-ов.
