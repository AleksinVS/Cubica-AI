...
4. Продолжать manifest-driven migration небольшими bounded slices: после закрытия `GSR-025` ближайший открытый slice теперь начинается с mainline boundary `stepIndex = 28`.
5. Переход `first board -> i7 -> second board 7..12 -> i8 -> board 13..18 -> i9 -> step 15 -> i10 -> board 19..24 -> i11 -> board 25..30 -> i12 -> board 31..36 -> i13 -> board 37..42 -> i14 -> i14_2 -> board 43..48 -> i15` уже покрыт на manifest boundary level, включая bounded line switch на loss line `i34 -> i34_2 -> i21`, locked/unlocked go-card `39`, entry-time alt `3902` и explicit public-communication board progression. Следующая естественная точка входа теперь - step `28` с cards `49..54`.
...
2. Для Antarctica maintain the bounded manifest-driven slices from `GSR-020`, `GSR-021`, `GSR-022`, `GSR-023` и `GSR-025` под архитектурными ограничениями `ADR-024`; post-confirm progression now reaches `stepIndex = 28`, step `21` is implemented with explicit `i12.advance`, explicit cards `31..36`, post-base conditional metric gates, bounded card-34 line switch, explicit loss-line continuation `i34 -> i34_2 -> i21`, step `23` is implemented with explicit cards `37..42`, locked go-card `39`, bounded unlock39-style threshold, entry-time alt swap `39 -> 3902`, and explicit `i14 -> i14_2`, while step `26` is implemented with explicit cards `43..48`, bounded metric hooks, explicit go-card follow-up to `i15`, and explicit `i15.advance`. Следующий открытый mainline boundary теперь - step `28` с cards `49..54`. Public shape for the team-selection slice remains `state.public.flags.team[memberId].selected`, `state.public.teamSelection.pickCount`, `state.public.teamSelection.selectedMemberIds`.
